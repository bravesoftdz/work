	...
	with initial_price as (
		...
		left join (
			select TM.IDModel, TQ.MinQuantity as MinQuantity, min(TQ.Discount) as QtyBreakPriceDiscount
				...
				and TQ.MinQuantity <= (select (abs(sum(qty))+abs(@qty)) as sumqty from preinventorymov where idpreinventorymov = @IDPreInventoryMov and InventMovTypeID = 1)
...
update PreInventoryMov set Qty = 15 where IDPreInventoryMov = 1200001002;

exec sp_PreSale_CalcPrice
    @IDModel = 1100000001,
	@StoreID = 1,
	@Qty = 5,
	@IDPreInventoryMov = 1200001002;

-- select SalePrice, UnitDiscount from PreInventoryMov where IDPreInventoryMov = 1200001002;


select *
from TagModel TM
join TagQtyPriceBreak TQ on (TM.IDTag = TQ.IDTag)
where TM.IDModel =  1100000001
    and TQ.MinQuantity <= (
        select (abs(sum(qty))+abs(5)) as sumqty
        from preinventorymov
        where idpreinventorymov = 1200001002 and InventMovTypeID = 1 
    )
;
...
join TagQtyPriceBreak TQ on (TM.idtag = TQ.idtag)
where TM.IDModel =  1100000001 
and TQ.MinQuantity <= (select (abs(sum(qty))+abs(5)) as sumqty from preinventorymov where idpreinventorymov = 1200001002 and InventMovTypeID = 1)
group by TM.IDModel, TQ.MinQuantity;
