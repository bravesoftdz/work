...
GO
ALTER PROCEDURE [dbo].[sp_PreSale_Pay]
			(
			@PreSaleID		int,
			@IDCashRegMov		int,
...
/* ---------------------------------------------------------------------------------------------------------------------------------------
	...
	10 Aug	2010		Rodrigo Costa		Tratar 4 casas decimais para configuracao da empresa
	---------------------------------------------------------------------------------------------------------------------------------- */
...
BEGIN
	SELECT @IsIndustry = CASE WHEN UPPER(SrvValue) = 'TRUE' THEN 1 ELSE 0 END FROM Param (NOLOCK) WHERE IDParam = 116
	...
	END
	ELSE
	IF (Round(@TotalInvoice, 2) <> @TotalPaid)
	BEGIN
		SET @ErrorLevel = -221
		GOTO ERRO
	END
