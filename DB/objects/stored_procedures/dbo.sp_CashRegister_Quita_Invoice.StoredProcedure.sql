...
GO
CREATE   PROCEDURE [dbo].[sp_CashRegister_Quita_Invoice]
	(
	@IDCashRegMov	int,
	@IDUsuario	int,
	@Date		Datetime
...
/* ----------------------------------------------------------------------------------------------------------------------
	...
	20 Jul	2009		Rodrigo Costa		Listar as quitacoes por PrazoRecebimento e nao por data lancalmento <> Data Fechamento
	----------------------------------------------------------------------------------------------------------------- */
...
DECLARE Fin_Quitacao_Cursor CURSOR FOR 
	...
	WHERE
		...
		AND ((IsNull(MPB.PrazoRecebimento, 0) = 0 ) AND ((IsNull(MPTS.ReceivingDay, 0) = 0)))
