...
GO
ALTER PROCEDURE [dbo].[sp_Sys_DailyMaintenance]
			(
			@CalcABC	bit = 1,
			@CalcAvg	bit = 1,
			@DelHold	bit = 1
...
/* ---------------------------------------------------------------------------------------------------------------------------------------
	...
	TABELA DE ERROS PARA RETURN_VALUE
		...
		-209  Erro em Remover compras com problema
	...
	10 Dez	2009		Rodrigo Costa		Remover compras com problema
	---------------------------------------------------------------------------------------------------------------------------------- */
...
-------------------------------------------------------------------------------------------------
-- Remove Compras com problemas

DECLARE PurchaseDel_Cursor CURSOR STATIC FOR 
	SELECT
		IDPurchase
	FROM
		Pur_Purchase (NOLOCK) 
	WHERE
		IDFornecedor IS NULL
		OR
		IDStore IS NULL

OPEN PurchaseDel_Cursor

--Inicialização de PreInv_Cursor
FETCH NEXT FROM PurchaseDel_Cursor INTO
            @IDPurchase
...
BEGIN
	--Deletar os items
	EXEC sp_Purchase_Remove @IDPurchase
...
--fechamento do cursor
CLOSE PurchaseDel_Cursor
--Destruição do cursor
DEALLOCATE PurchaseDel_Cursor
