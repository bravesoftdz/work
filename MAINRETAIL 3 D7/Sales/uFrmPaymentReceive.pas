...
//    procedure btnPayCashClick(Sender: TObject);
    ...
    procedure btnPayCashClick(Sender: TObject);
    procedure btnPayBonusClick(Sender: TObject);
    procedure btnPayCheckClick(Sender: TObject);
    procedure btnPayCreditCardClick(Sender: TObject);
    procedure btnPayDebitCardClick(Sender: TObject);
    procedure btnPayGiftClick(Sender: TObject);
    procedure btnPayStoreCreditClick(Sender: TObject);
    procedure btnPayOtherClick(Sender: TObject);
    procedure btnPayStoreAccClick(Sender: TObject);
  private
    ...
    // Antonio 03/31/2017
    procedure ValidateCustomer(argIdPaymentType: Integer); // argIdPaymentType from table MeioPag.IdMeioPag
    procedure VerifySignedPayMethods();
    function ValidateDocumentType(): Boolean;
...
uses uFrmSearchCustomer, uMsgBox, uMsgConstant, uCharFunctions, uPassword,
  ...
  uGynboSyncClasses, uDebugFunctions, contnrs;
...
begin
  ...
  // Antonio 04/04/2017
  objPayType := dm.FPayMethodList.GetPayMethod(PAYMENT_TYPE_CASH);

  if ValidatePayment() then
    if (Amount <> 0) then
    begin
      InsertCash(objPayType.GetIdType, TXiButton(Sender).Tag);
      RefreshValues;
...
procedure TFrmPaymentReceive.InsertCustomerCredit(AIDPayment: Integer;
  AAmount: Currency);
begin
//  showmessage(format('Customer  Credit: %.2f ', [FCustomerCredit]));

  if ( FCustomerCredit = 0 ) then begin
     MsgBox('Verify Credit conditions to this customer', vbCritical + vbOkOnly);
     exit;
  end;

  if (FCustomerCredit <> 0) or (AAmount < 0) then begin
    ...
    if FSaleReceive.InsertPayment(PAYMENT_TYPE_CREDIT, AIDPayment,
       ...
       Now, AAmount) then begin
        ...
        // Update Balance to Customer Credit            
        FCustomerCredit := FCustomerCredit - AAmount;
    end else begin
            //MsgBox(FSaleReceive.ErrorMsg, vbInformation + vbOkOnly);
            ShowMessage(FSaleReceive.ErrorMsg);
        end;
...
begin
  ...
  (* to place according to payment_type button
...
begin
  ...
  // Antonio 04/05/2017
  case Key of
    VK_F1:  btnPayCashClick(self);
    VK_F2:  btnPayCheckClick(self);
    VK_F3:  btnPayCreditCardClick(self);
    VK_F4:  btnPayDebitCardClick(self);
    VK_F5:  btnPayGiftClick(self);
    VK_F6:  btnPayBonusClick(self);
    VK_F7:  btnPayStoreCreditClick(self);
    VK_F8:  btnPayOtherClick(self);
    VK_F9:  btnPayStoreAccClick(self);
...
begin
  ...
  *)
  ...
  begin
// ?    if ValidateInsertPayment(PAYMENT_TYPE_CARD, -1) then begin
...
begin
  ...
  if (IDMeioPag <> -1) { and ValidateInsertPayment(PAYMENT_TYPE_CARD, IDMeioPag)} then begin
...
procedure TFrmPaymentReceive.ValidateCustomer(argIdPaymentType: Integer);
begin
   try
       if StrToBool(GetPaymentTypeValues(argIdPaymentType, 'RequireCustomer'))
          and (FDocumentInfo.IDCustomer <= 1) then begin
          raise Exception.Create(MSG_CRT_NO_CUSTUMER);
       end;

       if ((FDocumentInfo.IDCustomer > 1)) and (not Password.HasFuncRight(90)) then begin
          if FDocumentInfo.Nonpayer and GetPaymentTypeValues(argIdPaymentType, 'ValidateNonpayer') then begin
             raise Exception.Create(MSG_CRT_CUSTOMER_NOTUSE_PAYMENT);
          end;
      end;
  except
...
begin
  inherited;
  try
     if ( not dm.FPayMethodList.FoundPayMethod(PAYMENT_TYPE_CASH) ) then begin
        //raise Exception.Create('Payment type Cash not found in the database.'); // move to unit that contais just messages.
        MsgBox('Payment type Cash not found in the database.',  vbCritical + vbOkOnly);
        exit;
     end;
     ...
     ValidateCustomer(PAYMENT_TYPE_CASH);

     InsertCash(PAYMENT_TYPE_CASH, Amount);
     ...
     on e: Exception do begin
        raise e.Create('Cash Payment failed: ' + e.Message);
     end;
  end;
end;

procedure TFrmPaymentReceive.VerifySignedPayMethods();
begin
   dm.getPaymentWay();

   // Type = CASH = 1
   btnPayCash.ColorGrad := $00BCBCBC;
   if ( not dm.FPayMethodList.FoundPayMethod(PAYMENT_TYPE_CASH) ) then begin
       btnPayCash.ColorGrad := clRed;
   end;

   // Type = Credit Card = 2
   btnPayCreditCard.ColorGrad := $00BCBCBC;
   if ( not dm.FPayMethodList.FoundPayMethod(PAYMENT_TYPE_CARD) ) then begin
       btnPayCreditCard.ColorGrad := clRed;
   end;

   // Type = Other = 3
   btnPayOther.ColorGrad := $00BCBCBC;
   if ( not dm.FPayMethodList.FoundPayMethod(PAYMENT_TYPE_OTHER) ) then begin
       btnPayOther.ColorGrad := clRed;
   end;

   // Type = Check = 4
   btnPayCheck.ColorGrad := $00BCBCBC;
   if ( not dm.FPayMethodList.FoundPayMethod(PAYMENT_TYPE_CHECK) ) then begin
       btnPayCheck.ColorGrad := clRed;
   end;


   // Type = Credit = 5
   btnPayStoreCredit.ColorGrad := $00BCBCBC;
   if ( not dm.FPayMethodList.FoundPayMethod(PAYMENT_TYPE_CREDIT) ) then begin
       btnPayStoreCredit.ColorGrad := clRed;
   end;

   // Type = Gift Card = 6
   btnPayGift.ColorGrad := $00BCBCBC;
   if ( not dm.FPayMethodList.FoundPayMethod(PAYMENT_TYPE_GIFTCARD) ) then begin
       btnPayGift.ColorGrad := clRed;
   end;

   // Type = Debit = 7
   btnPayDebitCard.ColorGrad := $00BCBCBC;
   if ( not dm.FPayMethodList.FoundPayMethod(PAYMENT_TYPE_DEBIT) ) then begin
       btnPayDebitCard.ColorGrad := clRed;
   end;

   // Type = Bonus Bucks = 8
   btnPayBonus.ColorGrad := $00BCBCBC;
   if ( not dm.FPayMethodList.FoundPayMethod(PAYMENT_TYPE_BONUSBUCK) ) then begin
       btnPayBonus.ColorGrad := clRed;
   end;

   // Type = Store Account = 9
   btnPayStoreAcc.ColorGrad := $00BCBCBC;
   if ( not dm.FPayMethodList.FoundPayMethod(PAYMENT_TYPE_STOREACCOUNT) ) then begin
       btnPayStoreAcc.ColorGrad := clRed;
   end;

end;

procedure TFrmPaymentReceive.btnPayBonusClick(Sender: TObject);
var
  payMethod: TPayMethod;
  lbPreDate: Boolean;
  validPayTypeRef: Integer; // a valid payment type reference to payment selected
begin
  try

     if ( not ValidateDocumentType() ) then begin
        Exit;
     end;

     if ( not dm.FPayMethodList.FoundPayMethod(PAYMENT_TYPE_BONUSBUCK) ) then begin
        MsgBox('Payment type Bonus Buck not found in the database.', vbCritical + vbOkOnly);
        exit;
     end;

     payMethod := dm.FPayMethodList.GetPayMethod(PAYMENT_TYPE_BONUSBUCK);

     ClearCardSwipeError();

     ValidatePayment();

     if ( not ValidateAmount() ) then begin
         exit;
     end;

     if ( FDocumentInfo.IDCustomer <= 1 ) then begin
        MsgBox(MSG_CRT_NO_CUSTUMER, vbCritical + vbOkOnly);
        btnCustomerClick(Self);
        Exit;
      end;

     validPayTypeRef := dm.FoundPaymentReferenceType(PAYMENT_TYPE_BONUSBUCK);
     ValidateCustomer(validPayTypeRef);
     InsertBonusBucks(payMethod.GetIdType());
     ...
     on e: Exception do begin
        raise e.Create('Bonus bucks payment failed: ' + e.Message);
     end;
  end;

end;

procedure TFrmPaymentReceive.btnPayCheckClick(Sender: TObject);
var
  liIdBaseType, liIdPayType: integer;
  lbPreDate: boolean;
  payMethod: TPayMethod;
begin
  try
     ...
     if ( not dm.FPayMethodList.FoundPayMethod(PAYMENT_TYPE_CHECK) ) then begin
        //raise Exception.Create('Payment type Check not found in the database.'); // move to unit that contais just messages.
        MsgBox('Payment type Check not found in the database.',  vbCritical + vbOkOnly);
        exit;
     end;


     payMethod := dm.FPayMethodList.GetPayMethod(PAYMENT_TYPE_CHECK);
     liIdBaseType := payMethod.GetIdBaseType();
     liIdPayType := payMethod.GetIdType();
     ...
     ValidateCustomer(PAYMENT_TYPE_CHECK);

     if ( CallListPayment(PAYMENT_TYPE_CHECK) ) then begin
         FFrmListPayments.Start(liIdBaseType,liIdPayType, lbPreDate)
     end else begin
              liIdBaseType := payMethod.GetIdBaseType();
              liIdPayType := payMethod.GetIdType();
              lbPreDate := false;
     ...
     if ( liIdPayType = -1 ) then begin
          MsgBox(MSG_CRT_PAYMENT_NOT_EXIST, vbCritical + vbOKOnly);
          exit;
     end else begin
             InsertCheck(liIdPayType, Amount, lbPreDate);
         end;
     ...
     on e: Exception do begin
        raise e.Create('Check Payment failed: ' + e.Message);
     end;
  end;

end;

procedure TFrmPaymentReceive.btnPayCreditCardClick(Sender: TObject);
var
  payMethod: TPayMethod;
  liIdBaseType, liIdPayType: Integer;
  lbPreDate: Boolean;
begin
  try
     if ( not dm.FPayMethodList.FoundPayMethod(PAYMENT_TYPE_CARD) ) then begin
        //raise Exception.Create('Payment type Credit Card not found in the database.'); // move to unit that contais just messages.
        MsgBox('Payment type Credit not found in the database.',  vbCritical + vbOkOnly);
        exit;
     end;

     payMethod := dm.FPayMethodList.GetPayMethod(PAYMENT_TYPE_CARD);
     liIdBaseType := payMethod.GetIdBaseType();
     liIdPayType := payMethod.GetIdType();
     ...
     ValidateCustomer(PAYMENT_TYPE_CARD);

     if ( CallListPayment(PAYMENT_TYPE_CARD) ) then begin
          if ( FFrmListPayments.Start( liIdBaseType,liIdPayType, lbPreDate) ) then begin
              InsertCreditCard(liIdPayType, Amount, lbPreDate, false);
          end;
      end else begin
               liIdBaseType := payMethod.GetIdBaseType();
               liIdPayType := payMethod.GetIdType();
     ...
     if ( liIdPayType = -1 ) then begin
          ...
          exit;
             ...
             InsertCreditCard(payMethod.GetIdBaseType(), Amount, lbPreDate, false);
         end;
     ...
     on e: Exception do begin
        raise e.Create('Credit Card Payment failed: ' + e.Message);
     end;
  end;

end;

procedure TFrmPaymentReceive.btnPayDebitCardClick(Sender: TObject);
var
  payMethodBase, payMethod: TPayMethod;
  lbPreDate: Boolean;
  liIdBaseType: Integer;
  liIdPayType: Integer;
begin
  try
     if ( not dm.FPayMethodList.FoundPayMethod(PAYMENT_TYPE_DEBIT) ) then begin
        //raise Exception.Create('Payment type Debit Card not found in the database.'); // move to unit that contais just messages.
        MsgBox('Payment type Debit Card not found in the databse.',  vbCritical + vbOkOnly);
        exit;
     end;

     payMethod := dm.FPayMethodList.GetPayMethod(PAYMENT_TYPE_DEBIT);
     ...
     // PAYMENT_TYPE_DEBIT - reference to debit
     // ValidateCustomer(PAYMENT_TYPE_DEBIT);
     ValidateCustomer(dm.FoundPaymentReferenceType(PAYMENT_TYPE_DEBIT));

(*
     // to future requirements
     if ( CallListPayment(PAYMENT_TYPE_DEBIT) ) then begin
          if ( FFrmListPayments.Start( liIdBaseType,liIdPayType, lbPreDate) ) then begin
//             if ( liIdBaseType = PAYMENT_TYPE_DEBIT

          end;
      end else begin
               InsertDebitCard(payMethod.GetIdBaseType(), Amount);
          end;
 *)

     InsertDebitCard(payMethod.GetIdBaseType(), Amount);

     FSaleReceive.SavePaymentTypeDebitCard := payMethod.GetIdBaseType();
     FSaleReceive.SaveIdPaymentTypeDebitCard := payMethod.GetIdType();
     ...
     on e: Exception do begin
        raise e.Create('Debit Card Payment failed: ' + e.Message);
     end;
  end;
end;

procedure TFrmPaymentReceive.btnPayGiftClick(Sender: TObject);
var
  payMethod: TPayMethod;
  lbPreDate: Boolean;
  liIdBaseType, liIdPayType: Integer;
begin
  try
     ...
     if ( not dm.FPayMethodList.FoundPayMethod(PAYMENT_TYPE_GIFTCARD) ) then begin
        //raise Exception.Create('Payment type Gift Card not found in the database.'); // move to unit that contais just messages.
        MsgBox('Payment type Gift Card not found in the database.',  vbCritical + vbOkOnly);
        exit; 
     end;

     payMethod := dm.FPayMethodList.GetPayMethod(PAYMENT_TYPE_GIFTCARD);
     liIdBaseType := payMethod.GetIdBaseType();
     liIdPayType := payMethod.GetIdType();
     ...
     ValidateCustomer(PAYMENT_TYPE_GIFTCARD);

     if ( CallListPayment(PAYMENT_TYPE_GIFTCARD) ) then begin
          FFrmListPayments.Start( liIdBaseType,liIdPayType, lbPreDate );
     end else begin
             liIdBaseType := payMethod.GetIdBaseType();
             liIdPayType := payMethod.GetIdType();
     ...
     if ( liIdPayType = -1 ) then begin
        ...
        exit;
             ...
             InsertGiftCard(liIdPayType, Amount, lbPreDate, false);
          end;
     ...
     on e: Exception do begin
        raise e.Create('Gift Card Payment failed: ' + e.Message);
     end;
  end;

end;

procedure TFrmPaymentReceive.btnPayStoreCreditClick(Sender: TObject);
var
  payMethod: TPayMethod;
  lbPreDate: Boolean;
  liIdBaseType, liIdPayType: Integer;
begin
  try
     ...
     if ( not dm.FPayMethodList.FoundPayMethod(PAYMENT_TYPE_CREDIT) ) then begin
        //raise Exception.Create('Payment type Store Credit not found in the database.'); // move to unit that contais just messages.
        MsgBox('Payment type Store Credit not found in the database.', vbCritical + vbOkOnly);
        exit;
     end;

     payMethod := dm.FPayMethodList.GetPayMethod(PAYMENT_TYPE_CREDIT);
     ...
     ValidateCustomer(payMethod.GetIdType());

     InsertCustomerCredit(payMethod.GetIdType(), Amount);
...
procedure TFrmPaymentReceive.btnPayOtherClick(Sender: TObject);
var
  payMethod: TPayMethod;
  lbPreDate: Boolean;
  liIdBaseType, liIdPayType: Integer;
begin
  try
     if ( not dm.FPayMethodList.FoundPayMethod(PAYMENT_TYPE_OTHER) ) then begin
        raise Exception.Create('Payment type Other not found in the database.'); // move to unit that contais just messages.
     end;

     payMethod := dm.FPayMethodList.GetPayMethod(PAYMENT_TYPE_OTHER);
     liIdBaseType := payMethod.GetIdBaseType();
     liIdPayType := payMethod.GetIdType();
     ...
     ValidateCustomer(PAYMENT_TYPE_OTHER);

     if ( CallListPayment(PAYMENT_TYPE_OTHER) ) then begin
          FFrmListPayments.Start( liIdBaseType,liIdPayType, lbPreDate)
     end else begin
                 liIdBaseType := payMethod.GetIdBaseType();
                 liIdPayType := payMethod.GetIdType();
     ...
     if ( liIdPayType = - 1 ) then begin
         ...
         exit;
              ...
              InsertOther(liIdPayType, Amount);
         end;
     ...
     on e: Exception do begin
        raise e.Create('Other Payment failed: ' + e.Message);
     end;
  end;

end;

procedure TFrmPaymentReceive.btnPayStoreAccClick(Sender: TObject);
var
  payMethod: TPayMethod;
  lbPreDate: Boolean;
  liIdBaseType, liIdPayType: Integer;
begin
  try
     if ( not dm.FPayMethodList.FoundPayMethod(PAYMENT_TYPE_STOREACCOUNT) ) then begin
        raise Exception.Create('Payment type Store Account not found in the database.'); // move to unit that contais just messages.
     end;

     payMethod := dm.FPayMethodList.GetPayMethod(PAYMENT_TYPE_STOREACCOUNT);
     ...
     InsertStoreAccount(payMethod.GetIdType, Amount);
     ...
     on e: Exception do begin
        raise e.Create('Store Account Payment failed: ' + e.Message);
     end;
  end;

end;

function TFrmPaymentReceive.ValidateDocumentType(): Boolean;
begin
   result := true;

   if ( FDocumentInfo.DocumentType = dtServiceOrder ) then begin
      MsgBox('Document Type is not an invoice.', vbCritical + vbOKOnly);
      result := false;
   end;
...
function TFrmPaymentReceive.ValidateAmount(): Boolean;
begin
   result := true;
   if ( ABS(Amount) > ABS(TotalDue) ) then
   begin
     MsgBox(MSG_INF_NOT_GRATER_TOTAL_DUE, vbCritical + vbOkOnly);
     Amount := TotalDue;
     result := false;
