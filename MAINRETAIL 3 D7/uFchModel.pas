...
type
  TFchModel = class(TbrwFrmParent)
    ...
    quFormFreightCost: TFloatField;
    quFormOtherCost: TFloatField;
    quFormVendorCost: TFloatField;
    quFormDateLastCost: TDateTimeField;
    quFormMarkUp: TFloatField;
    quFormSellingPrice: TFloatField;
    quFormDateLastSellingPrice: TDateTimeField;
    quFormPeso: TFloatField;
    quFormIDFabricante: TIntegerField;
    quFormSystem: TBooleanField;
    quFormIDUnidade: TIntegerField;
    quFormRealMarkUpPercet: TFloatField;
    ...
    quFormES: TFloatField;
    quFormPVD: TFloatField;
    quFormPP: TFloatField;
    ...
    quFormTotQtyOnHand: TFloatField;
    ...
    chkDoNotUpdateCatalog: TDBCheckBox;
    quFormNoUpdateCatalogs: TBooleanField;
    tagsPromo: TTabSheet;
    panelTagPromoMain: TPanel;
    panelByTag: TPanel;
    panelTagPromoSpace: TPanel;
    panelByPromo: TPanel;
    panelByTagTop: TPanel;
    panelByPromoTop: TPanel;
    byTagGrid: TcxGrid;
    byTagGridTableView: TcxGridDBTableView;
    cxGridLevel2: TcxGridLevel;
    byTagGridTableViewDBColumn1: TcxGridDBColumn;
    byTagGridTableViewDBColumn2: TcxGridDBColumn;
    addTag: TSpeedButton;
    removeTag: TSpeedButton;
    cxGrid2: TcxGrid;
    cxGridDBTableView1: TcxGridDBTableView;
    cxGridLevel3: TcxGridLevel;
    dsByTag: TDataSource;
    dsByPromo: TDataSource;
    ...
    Function SimpleCaracter( Key : Char ) : Boolean;
  ...
  private
    promoByModel: TClientDataset;
    tagsByModel: TClientDataset;
    ...
    function getPromosByModel(arg_idmodel: Integer): TClientDataset;
    function getTagsByModel(arg_idmodel: Integer): TClientDataset;
...
uses uDM, xBase, uPassword, uModelAdjustQty, uMsgBox, uNewSerial, uCharFunctions,
     ...
     uFrmModelStateTax, uFrmAddNutrition, TagAvailableView;
...
begin
  ...
  editModel.ReadOnly := ( frmCommand = btAlt );
...
begin
  ...
  chkDoNotUpdateCatalog.Enabled := ( DM.fSystem.VersionType <> SYSTEM_CLIENT_TYPE );
...

 {
  ...
  begin
    ...
    if bUseMarkupOnCost then
      ...
      SellingPrice := DM.FDMCalcPrice.FormatPrice(DM.FDMCalcPrice.CalcRounding(quFormGroupID.AsInteger, sellingPrice));
(* except to manualy
...
begin
  ...
  begin
    ...
    Open;
    getPromosByModel(MyStrToInt(scLookUp.LookUpValue));
    getTagsByModel(MyStrToInt(scLookUp.LookUpValue));
  end;
...
begin
  ...
  freeAndNil(promoByModel);
  freeAndNil(tagsByModel);
...
begin
  ...
  begin
    ...
    begin
      ...
      scLookUpSelectItem(nil);
      getPromosByModel(IIDModel);
      getTagsByModel(IIDModel);
...
//  _saveId :=  quFormIDModel.asInteger;
...
//  quForm.edit;
//  quFormIDModel.AsInteger := _saveId;
...
procedure TFchModel.EditModelKeyPress(Sender: TObject; var Key: Char);
begin
  inherited;

  If ( SimpleCaracter( Key ) = false )
  Then Key := #0;

end;

Function TFchModel.SimpleCaracter( Key : Char ) : Boolean;
Begin

  Result := False;

  If ( Key In ['0'..'9'] ) Then Result := True;
  If ( Pos( UpCase( Key ), 'ABCDEFGHIJKLMNOPQRSTUVWXYZ' ) > 0 ) Then Result := True;
  If ( ( Key = #13 ) Or ( Key = #8 ) ) Then Result := True;

End;

function TFchModel.getPromosByModel(arg_idmodel: Integer): TClientDataset;
begin
  result := dm.promosByModel(arg_idmodel);
  promoByModel := result;
  dsByPromo.dataset := promoByModel;
end;

function TFchModel.getTagsByModel(arg_idmodel: Integer): TClientDataset;
begin
  result := dm.tagsByModel(arg_idmodel);
  tagsByModel := result;
  dsByTag.DataSet := tagsByModel;
end;

procedure TFchModel.removeTagClick(Sender: TObject);
begin
  inherited;
    // must be passed pktagmodel according to record selected.
    dm.callSpTagModelDelete(tagsByModel.fieldbyname('PKTagModel').Value);
   tagsByModel.Refresh();
   promoByModel.Refresh();
   //getPromosByModel(quformIdModel.value);
end;

procedure TFchModel.addTagClick(Sender: TObject);
var
  tagAvailableView: TtagsAvailableView;
begin
  inherited;

  try
    tagAvailableView := TtagsAvailableView.create(self);
    tagAvailableView.start(quFormIDModel.Value);
  finally
    freeAndNil(tagAvailableView);
  end;

  tagsByModel.Refresh();
  promoByModel.refresh();
  //getTagsByModel(quformIdModel.value);
  //getPromosByModel(quformIdModel.value);

end;

procedure TFchModel.OnAfterStart;
begin
  inherited;

  editModel.Color := clWindow;

  // to get idmodel selected by browse
  if ( (frmCommand = btAlt) and (quForm.Active) ) then begin
      getPromosByModel(quFormIDModel.AsInteger);
      getTagsByModel(quFormIDModel.AsInteger);
      editModel.Color := clMenu;
  end;
end;

procedure TFchModel.btNextRecClick(Sender: TObject);
begin
  inherited;
  quform.Next();
  onAfterStart();
...
procedure TFchModel.btLastRecClick(Sender: TObject);
begin
  inherited;
  quform.Last;
  onAfterStart();
...
procedure TFchModel.btFirstRecClick(Sender: TObject);
begin
  inherited;
  quform.First;
  onAfterStart();
...
procedure TFchModel.btPriorRecClick(Sender: TObject);
begin
  inherited;
  quform.Prior;
  onAfterStart();
...
procedure TFchModel.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  //inherited;
