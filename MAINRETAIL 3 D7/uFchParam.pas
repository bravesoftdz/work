...
type
  TFchParameter = class(TFrmParent)
    ...
    Label57: TLabel;
    lbProcessingType: TLabel;
    cmbProcessingType: TComboBox;
    loadParamEMV: TButton;
    Label58: TLabel;
    edtSAFooterPath: TEdit;
    SpeedButton11: TSpeedButton;
    btnSAFooterPath: TSpeedButton;
    SpeedButton13: TSpeedButton;
    labelTerminalID: TLabel;
    editTerminalID: TEdit;
    ...
    procedure cmbProcessingTypeCloseUp(Sender: TObject);
    procedure loadParamEMVClick(Sender: TObject);
    procedure btnSAFooterPathClick(Sender: TObject);
  ...
  private
    ...
    procedure SavePrice;
    procedure EnableDisabledTerminalID(arg_isEMV: Boolean);
  public
...
uses uDM, uMsgBox, uMsgConstant, uDMGlobal, MenuPrincipal, cxLookAndFeels,
     ...
     uFrmPrintStoreAccount, uGynboSyncClasses, uOperationSystem, uCCMercuryIntegration;
...
begin
  ...
  with TRegistry.Create do
    begin
      ...
      // Antonio 01/28/2016
      edtSAFooterPath.Text         := ReadString('StoreAccReportFooterPath');
      ...
      // Antonio, July 18, 2013 - MercuryGift Cards
      edtMercGiftPort.Text			   := IntToStr(readInteger('MercuryGiftIPPort'));
      edtMercMechantID.text         := readString('MercuryGiftMerchantID');
      edtMercIPAddress.Text         := readString('MercuryGiftIPs');

      cbbGCProcessor.ItemIndex       := ReadInteger('GiftProcessor');
      ...
      cmbProcessingType.ItemIndex    := readInteger('ProcessType');

      // get Terminal ID parameter
      editTerminalID.Text := ReadString('TerminalID');
...
begin
  ...
  begin
    ...
    try
      ...
      // Antonio 01/28/2016
      writeString('StoreAccReportFooterPath', trim(edtSAFooterPath.Text));
      ...
      // Antonio July 18, 2013 to fix MerchantID to Mercury
      WriteString('MercuryGiftMerchantID', edtMercMechantID.Text);
//      WriteInteger('MercuryConnectTimeout', StrToInt(edtMerConTimeOut.Text));
//      WriteInteger('MercuryResponseTimeout', StrToInt(edtResponseTimeout.Text));
      ...
      // Processing Type
      WriteInteger('ProcessType', cmbProcessingType.ItemIndex);
      WriteString('TerminalID', editTerminalID.Text);
...
begin
  ...
  // Antonio 05/26/2016: default settings
  case ( cbxProcessors.itemIndex ) of
      NO_PROCESSOR: begin
         cbbGCProcessor.ItemIndex := NO_PROCESSOR;
         cmbProcessingType.ItemIndex := PROCESSING_TYPE_MANUAL_CHARGE;
         loadParamEMV.Enabled := false;
      end;
  end;


  // Antonio 05/26/2016
  pgCharge.Visible := ( cbxProcessors.ItemIndex in [PROCESSOR_MERCURY, PROCESSOR_WORLDPAY] );
  if ( pgCharge.Visible ) then begin
      ntProcessor.PageIndex := (cbxProcessors.ItemIndex);
      pnGCProcessor.Visible := cbxProcessors.ItemIndex in [PROCESSOR_MERCURY, PROCESSOR_WORLDPAY]; // WorldPay added.
...
begin
  ...
  // Antonio July 18, 2013 - tab default
  pgCharge.ActivePage := tsCardSetup;
  ...
  with DM.quFreeSQL do
    ...
    grdPayment.Cells[1,0]  := 'Card Type';
...
procedure TFchParameter.cmbProcessingTypeCloseUp(Sender: TObject);
var
   widget: TCCreditDSIEMVx;
   emvParamResult: Boolean;
begin
  inherited;

  // 05/26/2016
  loadParamEMV.Enabled := cmbProcessingType.ItemIndex in [PROCESSING_TYPE_DSIEMVUS, PROCESSING_TYPE_DSIPDCX];
  EnableDisabledTerminalID(loadParamEMV.Enabled);  

end;

procedure TFchParameter.loadParamEMVClick(Sender: TObject);
var
   widget: TCCreditDSIEMVx;
   emvParamResult: Boolean;
begin
  inherited;
  try
     widget := TCCreditDSIEMVx.create();
     widget.SequenceNo := '';
     emvParamResult := widget.EMVParamDownload();
     if ( not emvParamResult ) then begin
        showMessage('EMVParamDownloadFailed');
     end;
  finally
    freeAndNil(widget);
  end;
...
procedure TFchParameter.btnSAFooterPathClick(Sender: TObject);
begin
  OP.Filter := FILE_REPORT;
  if OP.Execute then
     edtSAFooterPath.Text := OP.FileName;
...
procedure TFchParameter.EnableDisabledTerminalID(arg_isEMV: Boolean);
begin
   editTerminalID.Enabled := false;
   labelTerminalID.Enabled := false;

   if ( arg_isEMV ) then begin
       if ( cbxProcessors.ItemIndex in [PROCESSOR_MERCURY, PROCESSOR_WORLDPAY] ) then begin
          editTerminalID.Enabled := true;
          labelTerminalID.Enabled := true;
       end;
   end;    
end;
