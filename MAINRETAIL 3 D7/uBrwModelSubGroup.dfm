inherited brwModelSubGroup: TbrwModelSubGroup
  Left = 315
  Top = 159
  Height = 523
  Caption = 'brwModelSubGroup'
  PixelsPerInch = 96
  TextHeight = 13
  inherited Panel1: TPanel
    Top = 455
  end
  inherited EspacamentoEsquerdo: TPanel
    Height = 421
  end
  inherited EspacamentoDireito: TPanel
    Height = 421
  end
  inherited pnlMain: TPanel
    Height = 421
    inherited pnlBasicFilter: TPanel
      inherited pblGO: TPanel
        inherited pnlAviso: TPanel
          inherited btnExec: TSpeedButton
            Visible = False
          end
        end
      end
    end
    inherited pnlButton: TPanelRights
      Height = 385
      inherited imgFolder: TImage
        Top = 315
      end
      inherited ImgTrash: TImage
        Top = 350
      end
    end
    inherited pnlMainBrowse: TPanel
      Height = 385
      inherited pnlPage: TPanel
        Top = 366
      end
      inherited grdBrowse: TcxGrid
        Height = 338
        inherited grdBrowseDB: TcxGridDBTableView
          DataController.Filter.Criteria = {FFFFFFFF0000000000}
          DataController.KeyFieldNames = 'IDModelSubGroup'
          OptionsView.Footer = False
          object grdBrowseDBIDModelSubGroup: TcxGridDBColumn
            Caption = 'ID'
            DataBinding.FieldName = 'IDModelSubGroup'
            Width = 45
          end
          object grdBrowseDBModelSubGroup: TcxGridDBColumn
            DataBinding.FieldName = 'ModelSubGroup'
            Width = 217
          end
          object grdBrowseDBName: TcxGridDBColumn
            Caption = 'Category'
            DataBinding.FieldName = 'Name'
            Width = 108
          end
          object grdBrowseDBModelGroup: TcxGridDBColumn
            DataBinding.FieldName = 'ModelGroup'
            Width = 137
          end
          object grdBrowseDBUserCode: TcxGridDBColumn
            DataBinding.FieldName = 'UserCode'
            Visible = False
            Width = 93
          end
        end
      end
    end
  end
  inherited siLang: TsiLangRT
    StorageFile = 'MR_BrwModelSubGroup.sil'
    TranslationData = {
      }
  end
  inherited quBrowse: TPowerADOQuery
    CommandText = 
      'SELECT'#13#10#9'MS.IDModelSubGroup,'#13#10#9'MS.ModelSubGroup,'#13#10#9'MS.SalePriceM' +
      'argemPercent,'#13#10#9'MS.UseSalePricePercent,'#13#10#9'MS.IDSalePriceMargemTa' +
      'ble,'#13#10#9'MS.IDMSRPMargemTable,'#13#10#9'MS.MSRPMargemPercent,'#13#10#9'MS.UseMSR' +
      'PPercent,'#13#10#9'MS.UserCode,'#13#10#9'MG.ModelGroup,'#13#10#9'TG.Name'#13#10'FROM'#13#10#9'Mode' +
      'lSubGroup MS (NOLOCK)'#13#10#9'LEFT JOIN ModelGroup MG (NOLOCK) ON (MG.' +
      'IDModelGroup = MS.IDModelGroup) '#13#10#9'LEFT JOIN TabGroup TG (NOLOCK' +
      ') ON (TG.IDGroup = MG.IDGroup)'
    object quBrowseIDModelSubGroup: TIntegerField
      FieldName = 'IDModelSubGroup'
    end
    object quBrowseModelSubGroup: TStringField
      FieldName = 'ModelSubGroup'
      Size = 30
    end
    object quBrowseSalePriceMargemPercent: TFloatField
      FieldName = 'SalePriceMargemPercent'
    end
    object quBrowseUseSalePricePercent: TBooleanField
      FieldName = 'UseSalePricePercent'
    end
    object quBrowseIDSalePriceMargemTable: TIntegerField
      FieldName = 'IDSalePriceMargemTable'
    end
    object quBrowseIDMSRPMargemTable: TIntegerField
      FieldName = 'IDMSRPMargemTable'
    end
    object quBrowseMSRPMargemPercent: TFloatField
      FieldName = 'MSRPMargemPercent'
    end
    object quBrowseUseMSRPPercent: TBooleanField
      FieldName = 'UseMSRPPercent'
    end
    object quBrowseModelGroup: TStringField
      FieldName = 'ModelGroup'
      Size = 30
    end
    object quBrowseName: TStringField
      FieldName = 'Name'
      Size = 30
    end
    object quBrowseUserCode: TStringField
      FieldName = 'UserCode'
      Size = 5
    end
  end
  inherited gridPrinter: TdxComponentPrinter
    PreviewOptions.PreviewBoundsRect = {00000000000000002003000058020000}
    inherited gridPrinterLink: TdxGridReportLink
      DateTime = 38663.668387766210000000
      PrinterPage.Footer = 363
      PrinterPage.Header = 20
      PrinterPage.Margins.Bottom = 12700
      PrinterPage.Margins.Left = 12700
      PrinterPage.Margins.Right = 12700
      PrinterPage.Margins.Top = 12700
      PrinterPage.PageSize.X = 210007
      PrinterPage.PageSize.Y = 297002
      PrinterPage._dxMeasurementUnits_ = 0
      PrinterPage._dxLastMU_ = 1
      BuiltInReportLink = True
    end
  end
end
