...
type
  ...
  TFrmBarcodePrint = class(TFrmParentAll)
    ...
    quBarcodeListLargeImage: TStringField;
    quBarcodeListLargeImage2: TStringField;
    quBarcodeListManufacturer: TStringField;
    quBarcodeListVendorNum: TStringField;
    quBarcodeListVendor: TStringField;
    cdsBarcodeListLargeImage: TStringField;
    cdsBarcodeListLargeImage2: TStringField;
    cdsBarcodeListManufacturer: TStringField;
    cdsBarcodeListVendorNum: TStringField;
    cdsBarcodeListVendor: TStringField;
    quBarcodeListSizeName: TStringField;
    quBarcodeListColor: TStringField;
    quBarcodeListCodColor: TStringField;
    ppDBText5: TppDBText;
    ppDBText6: TppDBText;
    cdsBarcodeListSizeName: TStringField;
    cdsBarcodeListColor: TStringField;
    cdsBarcodeListCodColor: TStringField;
    quBarcodeListModelToScan: TStringField;
    quBarcodeListBarcodeToScan: TStringField;
    cdsBarcodeListModelToScan: TStringField;
    cdsBarcodeListBarcodeToScan: TStringField;
    ppDBText7: TppDBText;
...
begin
  ...
  with dsBarcCode.DataSet do
     ...
     While not EOF do
        begin
        // AmfSouza November 07, 2012 new parameters
        AddBarcode(FieldByName('IDBarcode').AsString,
                   ...
                   FieldByName('LargeImage').asString,          // new
                   FieldByName('LargeImage2').asString,         // new
                   FieldByName('Manufacturer').asString,        // new
                   FieldByName('VendorNum').asString,           // new
                   FieldByname('Vendor').asString,              // new

                   // AmfSouza November 08, 2012: new fields to get size and color
                   FieldByName('SizeName').asString,
                   FieldByName('Color').AsString,
                   FieldByName('CodColor').AsString,

                   // AmfSouza November 12, 2012: new fields to allow use scan
                   FieldByName('ModelToScan').asString,
                   FieldByName('BarcodeToScan').asString
                   );
        Next;
        end;
     EnableControls;
...
//*) //**************************************
          ...
          begin
            ...
            //sSelect := ', StoreAVGPrice, (CASE IsNull(I.SellingPrice, 0) WHEN 0 THEN M.SellingPrice ELSE I.SellingPrice END) as SellingPrice, I.QtyOnHand ';
            sJoin   := 'LEFT JOIN Inventory I ON (I.ModelID = BA.IDModel) ';  //YAA --> This was removed. I put it back //sJoin   := 'LEFT JOIN Inventory I ON (I.ModelID = BA.IDModel) ';
            //YAA May 2nd, 2012:   Model price changes need to show up across all stores, not just corporate
            // TODO: Inventory price changes should only show up at store where  price was changed.
            // where statement below commented out  as better to show extra than not enough
            //sWhere  := sWhere + ' AND I.StoreID = ' + InttoStr(DM.fStore.ID); //YAA --> This was removed. I put it back // + ' AND IsNull(I.QtyOnHand,0) > 0'; //sWhere  := sWhere + ' AND I.StoreID = ' + InttoStr(DM.fStore.ID);// + ' AND IsNull(I.QtyOnHand,0) > 0';
          end
          else begin
            sSelect := '  (CASE IsNull(I.SellingPrice, 0) WHEN 0 THEN M.SellingPrice ELSE I.SellingPrice END) as SellingPrice, (select QtyOnHand from Inventory I where I.ModelID = M.IDModel and I.StoreID = '+InttoStr(DM.fStore.ID)+ ') as QtyOnHand,'+//YAA --> ' as QtyOnHand,'+
...
//        setToPrintQueue;
  ...
  case pgFilter.ActivePageIndex of
    0: begin
      ...
      cdsBarcodeList.Open;
      grdBarcodeDb.DataController.DataSource := dsBarcodeList;
      lbListTab.Caption := 'Search Results = '+ intToStr(cdsBarcode.RecordCount);
    end;
    1: begin
      ...
      cdsBarcodeList.Open;
      grdBarcodeDb.DataController.DataSource := dsBarcodeList;
      lbLastUpdateTab.Caption := 'Search Results = '+ intToStr(cdsBarcode.RecordCount);
    end;
...
' C.Color, '+
' C.CodColor, '+
// AmfSouza November 12, 2012: added to allow use of scan
' (''*'' + LTRIM(RTRIM(M.Model ))+ ''*'') as ModelToScan, '+
' (''*'' + LTRIM(RTRIM(BA.IdBarcode)) + ''*'') as BarcodeToScan, '+

'	IsNull(M.DateLastSellingPrice,'+ DateToStr(strToDate('01/01/2000'))+') as Data ' ;
...
//AfSouza November 06, 2012'   LEFT JOIN Inv_ModelVendor IMV (NOLOCK) ON (M.IDModel = IMV.IDModel) ' +
'   LEFT JOIN Inv_ModelVendor MV ON (M.IdModel = MV.IdModel and MV.VendorOrder = 1) '+
'   LEFT JOIN VendorModelCode vmc (NOLOCK) on M.idModel = vmc.idModel   ' +
//AmfSouza November 06, 2012'   LEFT JOIN Pessoa P (NOLOCK) ON vmc.IdPessoa = P.IDPessoa and M.IdModel = vmc.IdModel ';
'   LEFT JOIN PESSOA F ON (M.IdFabricante = F.IdPessoa) '+
'   LEFT JOIN PESSOA V ON (MV.IdPessoa = V.IdPessoa ) ';
  ...
  if IDPurchase <> ''then
     ...
     with DM.quFreeSQL do
        ...
        SQL.Text := 'SELECT P.IDPurchaseItem, P.IDModel, P.Barcode, M.Model, B.IDBarCode, P.Qty, M.Description, P.qty as QtyOnHand, ' +
                    ...
                    // AmfSouza November 09, 2012: new added fields
                    ' M.LargeImage, M.LargeImage2, F.Pessoa as Manufacturer, ' +
                    ' V.UserCode as VendorNum, V.Pessoa as Vendor, S.SizeName,'+
                    ' C.Color, C.CodColor,(''*''+ LTRIM(RTRIM(M.Model))+''*'') as ModelToScan, (''*''+LTRIM(RTRIM(B.IdBarcode))+''*'') as BarcodeToScan '+
                    ...
                    // AmfSouza November 09, 2012: new fields added
                    ' LEFT OUTER JOIN InvSize S (NOLOCK) ON (S.IDSize = M.IDSize) '+
	                  ' LEFT OUTER JOIN InvColor C (NOLOCK) ON (C.IDColor = M.IDColor) '+
          ...
          begin
             if FieldByName('IdBarcode').AsString <> '' then
                AddBarcode(FieldByName('IdBarcode').AsString,
                           ...
                           FieldByName('QtyOnHand').AsFloat,
                           FieldByname('StoreAVGPrice').AsCurrency,
                           FieldByName('LargeImage').asString,
                           FieldByName('LargeImage2').asString,
                           FieldByName('Manufacturer').asString,
                           FieldByName('VendorNum').asString,
...
begin
  ...
  // AmfSouza November 07, 2012: New paramters added
  with dsBarcCode.DataSet do
  begin
  ...
  try
      if ( hasSearch(dm.fUser.ID) ) then begin
         ...
         while ( not qrySearch.Eof ) do begin
            ...
            // AmfSouza November 07, 2012
            cdsBarcodeList.FieldByName('LargeImage').Value := qrySearch.fieldByname('LargeImage').Value;
            cdsBarcodeList.FieldByName('LargeImage2').Value := qrySearch.fieldByname('LargeImage2').Value;
            cdsBarcodeList.FieldByName('Manufacturer').Value := qrySearch.fieldByname('Manufacturer').Value;
            cdsBarcodeList.FieldByName('VendorNum').Value := qrySearch.fieldByname('VendorNum').Value;
            cdsBarcodeList.FieldByName('Vendor').Value := qrySearch.fieldByname('Vendor').Value;

            // AmfSouza November 08, 2012
            cdsBarcodeList.FieldByName('SizeName').value := qrySearch.fieldByName('SizeName').Value;
            cdsBarcodeList.FieldByName('Color').value := qrySearch.fieldByName('Color').Value;
            cdsBarcodeList.FieldByName('CodColor').value := qrySearch.fieldByName('CodColor').Value;

            // AmfSouza November 12, 2012
            cdsBarcodeList.fieldByname('ModelToScan').value := qrySearch.fieldByName('ModelToScan').value;
            cdsBarcodeList.fieldByname('BarcodeToScan').value := qrySearch.fieldByName('BarcodeToScan').value;
...
begin
    ...
    sSQL := 'SELECT distinct M.IDModel '+
                    ...
                    ',msg.modelsubgroup msgGroup' +
                    ',M.Description '+
//AmfSouza November 06, 2012 ',m.DateLastSellingPrice ' ;
                    ', M.DateLastSellingPrice '+
                    ', M.LargeImage '+
                    ', M.LargeImage2 '+
                    ', F.Pessoa as Manufacturer '+
                    ...
                    // AmfSouza November 12, 2012
                    ', (''*''+LTRIM(RTRIM(M.Model))+''*'') as  ModelToScan ' +
                    ', (''*''+LTRIM(RTRIM(BA.IdBarcode))+''*'')BarcodeToScan ';
              ...
              'FROM Barcode BA (NOLOCK) ' +
                    ...
                    // AmfSouza November 08, 2012: added to get size and color data
                    'LEFT OUTER JOIN InvSize S (NOLOCK) ON (S.IDSize = M.IDSize) '+
	                  'LEFT OUTER JOIN InvColor C (NOLOCK) ON (C.IDColor = M.IDColor) '+

//Amfsouza November 06, 2012 'LEFT JOIN Inv_ModelVendor IMV (NOLOCK) ON (M.IDModel = IMV.IDModel) ' +
                    'LEFT JOIN Inv_ModelVendor MV ON (M.IdModel = MV.IdModel and MV.VendorOrder = 1) '+
...
//AmfSouza November 06, 2012 sSQL := ssQL + ' LEFT JOIN Pessoa P (NOLOCK) ON vmc.IdPessoa = P.IDPessoa and M.Idmodel = vmc.IdModel ';
                    sSQL := sSQl + ' LEFT JOIN PESSOA F ON (M.IdFabricante = F.IdPessoa) '+
                                   ' LEFT JOIN PESSOA V ON (MV.IdPessoa = v.IdPessoa) ';
    ...
    begin
        sWhere := 'WHERE vmc.vendorCode = :sku ' +
                    ' AND MV.IDPessoa = :idpessoa and m.desativado = 0 ' ;
    end;
  ...
  if ( chkSaveQueue.Checked ) then begin
      ...
      if ( not cdsBarcodeList.IsEmpty ) then begin
           try
               ...
               // AmfSouza November 07, 2012 - new fields due large images in other fields as from this one.
               ...
               'LargeImage, LargeImage2, Manufacturer, VendorNum, Vendor, SizeName, Color, CodColor, ModelToScan, BarcodeToScan) ' +
               ...
               sqlUpdate := 'Update Inv_BarcodeprintSearch set NumCopy = :numcopy, ' +
                ...
                'LargeImage = :largeimage, LargeImage2 = :largeimage2, Manufacturer = :manufacturer, VendorNum = :vendornum, Vendor = :vendor, ' +

                // AmfSouza November 08, 2012: added to get size and color
                'SizeName = :sizename, Color = :color, CodColor = :codcolor '+

                // AmfSouza November 12, 2012: added to allow to use of scan
                ',ModelToScan = :modeltoscan, BarcodeToScan = :barcodetoscan ' +
             ...
             try
               ...
               while ( not cdsBarcodeList.Eof ) do begin
                 ...
                 //**************************************
                  ...
                  // AmfSouza November 07, 2012 - new fields due large images in other fields as from this one.
                   if ( not cdsBarcodeList.FieldByName('LargeImage').IsNull ) then
                      qrySavePrint.Parameters.ParamByName('LargeImage').Value := cdsBarcodeList.fieldByName('LargeImage').asString;

                   if ( not cdsBarcodeList.FieldByName('LargeImage2').IsNull ) then
                      qrySavePrint.Parameters.ParamByName('LargeImage2').Value := cdsBarcodeList.fieldByName('LargeImage2').asString;

                   if ( not cdsBarcodeList.FieldByName('Manufacturer').IsNull ) then
                      qrySavePrint.Parameters.ParamByName('Manufacturer').Value := cdsBarcodeList.fieldByName('Manufacturer').asString;

                   if ( not cdsBarcodeList.FieldByName('VendorNum').IsNull ) then
                      qrySavePrint.Parameters.ParamByName('VendorNum').Value := cdsBarcodeList.fieldByName('VendorNum').asString;

                   if ( not cdsBarcodeList.FieldByName('Vendor').IsNull ) then
                      qrySavePrint.Parameters.ParamByName('Vendor').Value := cdsBarcodeList.fieldByName('Vendor').asString;

                   // AmfSouza November 08, 2012: added to get size and color
                   if ( not cdsBarcodeList.FieldByName('SizeName').IsNull ) then
                      qrySavePrint.Parameters.ParamByName('SizeName').Value := cdsBarcodeList.fieldByName('SizeName').asString;

                   if ( not cdsBarcodeList.FieldByName('Color').IsNull ) then
                      qrySavePrint.Parameters.ParamByName('Color').Value := cdsBarcodeList.fieldByName('Color').asString;

                   if ( not cdsBarcodeList.FieldByName('CodColor').IsNull ) then
                      qrySavePrint.Parameters.ParamByName('CodColor').Value := cdsBarcodeList.fieldByName('CodColor').asString;

                   // AmfSouza November 12, 2012: To allow to use of scan
                   if ( not cdsBarcodeList.FieldByName('ModelToScan').IsNull ) then
                      qrySavePrint.Parameters.ParamByName('ModelToScan').Value := cdsBarcodeList.fieldByName('ModelToScan').asString;

                   if ( not cdsBarcodeList.FieldByName('BarcodeToScan').IsNull ) then
                      qrySavePrint.Parameters.ParamByName('BarcodeToScan').Value := cdsBarcodeList.fieldByName('BarcodeToScan').asString;
