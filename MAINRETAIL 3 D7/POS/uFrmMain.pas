...
var
  ...
  sSenha, FTotalizadoresParciais, sMsgErro : String;
begin
  bResult := True;
  ...
  begin
    ...
    case DM.fCashRegister.UseTef of
      ...
      TEF_DEDICADO:
        begin
          ...
          Case iStatus of
            1 : sMsgErro := 'Endereço IP inválido ou não resolvido';
            2	: sMsgErro := 'Código da loja inválido';
            3	: sMsgErro := 'Código de terminal invalido';
            6	: sMsgErro := 'Erro na inicialização do Tcp/Ip';
            7	: sMsgErro := 'Falta de memória';
            8	: sMsgErro := 'Não encontrou a CliSiTef ou ela está com problemas';
            10:	sMsgErro := 'O PinPad não está devidamente configurado no arquivo CliSiTef.ini';
          end;

          if iStatus <> 0 then
            MsgBox('TEF Error (' + IntToStr(iStatus) + '): ' + sMsgErro, vbCritical + vbOkOnly);
        end;
    ...
    //PAF
    DM.EFiscal1.RetornaTotalizadorParcial(FTotalizadoresParciais);
    DM.fModuloFiscal.TotParciais := FTotalizadoresParciais;
   ...
   case DM.EFiscal1.Impressora of
     BematechTermica : DM.LoadBematechTermInfo;
     Elgin : DM.LoadElginTermInfo;
     Sweda03, Sweda10 : DM.LoadSwedaTermInfo;
   end;
