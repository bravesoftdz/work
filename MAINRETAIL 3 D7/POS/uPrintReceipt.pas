...
type
  ...
  private
    ...
    procedure ImpStoreAccount(FExpireDate:TDateTime ; FValue:Currency ; Customer: String);
...
begin
  ...
  begin
    ...
    for J := 0 to PMT.Count - 1 do
    begin
      if PMT.Tipo = PAYMENT_TYPE_STOREACCOUNT then
      begin
        PPay := PMT.Items[J];
        ImpStoreAccount(PPay.ExpireDate, PPay.Value, FPresale.PreSaleInfo.CustomerInfo.FirstName + ' ' + FPresale.PreSaleInfo.CustomerInfo.LastName);
      end;
    end;
  end;
  DM.PrinterStop;
...
procedure TPrintReceipt.ImpStoreAccount(FExpireDate:TDateTime ; FValue:Currency ; Customer: String);
begin
  DM.PrintLine('          Conta   da  Loja              ');
  DM.PrintLine('');
  DM.PrintLine('Data Impressão: ' + FormatDateTime('ddddd', Now));
  DM.PrintLine('Data Pagamento: ' + FormatDateTime('ddddd', FExpireDate));
  DM.PrintLine('Valor         :' + FormatCurr('#,###,##0.00', FValue));
  DM.PrintLine('');
  DM.PrintLine('');
  DM.PrintLine('');
  DM.PrintLine('');
  DM.PrintLine('________________________________________');
  DM.PrintLine('           ' + customer);
  DM.PrintLine('');
  DM.PrintLine('');
  DM.PrintLine('');
  DM.PrintLine('');
