...
type
  ...
  TFchParameter = class(TFrmParentAll)
    ...
    procedure chkImpressoraFiscalClick(Sender: TObject);
  private
    fMode : Integer;
...
uses uDM, uMsgBox, uMsgConstant, uDMGlobal, uSystemConst, Variants,
  VarConv, EFiscal, uFrmTaxPaymentConfig, uBalanca, uOperationSystem;

{$R *.DFM}

procedure TFchParameter.RefreshCupomFooter;
begin
  pnlFooter.Visible := (cbxPrinterFiscalName.ItemIndex in [20,21,22,23,24,25,26,27]);
...
begin
  ...
  try
    ...
    chkImpressoraFiscal.Checked   := ReadBool('FiscalPrinter');
    ...
    if (not cbOpenCashReg.Checked) or (not DM.fCashRegister.FiscalPrinter) then
    begin
  ...
  case cbxPrinterFiscalName.ItemIndex of
       ...
       22:Impressora:=BematechTermica;
       23:Impressora:=Procomp_2011;
       24:Impressora:=AFRAC;
       25:Impressora:=DarumaMatr;
       26:Impressora:=DarumaTerm;
       27:Impressora:=Elgin;
       28:Impressora:=SCFI;
       end;
  end;
...
begin
  ...
  begin
    ...
    try
      ...
      //Printer
      WriteBool('FiscalMode', chkImpressoraFiscal.Checked);
      ...
      CloseKey;
      OpenKey(REGISTRY_PATH_POS, True);
      WriteBool('FiscalPrinter', chkImpressoraFiscal.Checked);
...
begin
  ...
  tsPDVPrinter.TabVisible     := DM.fCashRegister.Fiscal;
  tsTEF.TabVisible            := tsPDVPrinter.TabVisible;
  chkImpressoraFiscal.Visible := (DMGlobal.IDLanguage = LANG_PORTUGUESE); 
end;
...
begin
  ...
  if DM.FMD5 <> '' then
  begin
    if DM.EFiscal1.ProgramaIdAplicativo('MainRetail - Cash Register 1.6', 'MD5: ' + DM.FMD5) then
      ShowMessage('Menssagem com MD5 registrada na impressora!');
  end
  else
...
procedure TFchParameter.chkImpressoraFiscalClick(Sender: TObject);
begin
  inherited;
  tsPDVPrinter.TabVisible := chkImpressoraFiscal.Checked;
  tsTEF.TabVisible        := tsPDVPrinter.TabVisible;
end;
