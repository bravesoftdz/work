...
uses xBase, PowerParam, uNumericFunctions;
...
begin
  ...
  if High(aFields) <> High(aValues) then
     raise exception.create('SuperCombo - erro na passagem do filtro')
  else
    begin
...
begin
  ...
  if not Assigned(FLookUpSource) then
     raise exception.create('SuperCombo ' + Name + ' - You must set the LookUpSource');
...
procedure TSuperComboADO.SetExit;
var
  OldText : String;
begin
  if Assigned(LookUpADOQuery) then
...
begin
  if not Assigned(LookUpADOQuery.OnClickButton) then
     raise exception.create('SuperCombo - Evento OnClickButton nao foi definido');
