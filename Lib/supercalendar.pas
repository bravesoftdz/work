...
uses Classes, Controls, Messages, Windows, Forms, Graphics, StdCtrls,
  Grids, SysUtils, Dialogs;

type
  TDayOfWeek = 0..6;

  TSuperCalendar = class(TCustomGrid)
  private
    FDate: TDateTime;
...
constructor TSuperCalendar.Create(AOwner: TComponent);
begin
...
procedure TSuperCalendar.Change;
begin
  if Assigned(FOnChange) then FOnChange(Self);
end;

procedure TSuperCalendar.Click;
var
  TheCellText: string;
begin
...
function TSuperCalendar.IsLeapYear(AYear: Integer): Boolean;
begin
  Result := (AYear mod 4 = 0) and ((AYear mod 100 <> 0) or (AYear mod 400 = 0));
end;

function TSuperCalendar.DaysPerMonth(AYear, AMonth: Integer): Integer;
const
...
function TSuperCalendar.DaysThisMonth: Integer;
begin
  Result := DaysPerMonth(Year, Month);
end;

procedure TSuperCalendar.DrawCell(ACol, ARow: Longint; ARect: TRect; AState: TGridDrawState);
var
  TheText: string;
begin
  ...
  with ARect, Canvas do
    begin
      if ( TheText     = IntToStr(nDay(Date)) ) and
         ( nYear(Date)  = nYear(FDate) )         and
         ( nMonth(Date) = nMonth(FDate) )        then
        begin
...
function TSuperCalendar.GetCellText(ACol, ARow: Integer): string;
var
  DayNum: Integer;
begin
...
function TSuperCalendar.SelectCell(ACol, ARow: Longint): Boolean;
begin
...
procedure TSuperCalendar.SetCalendarDate(Value: TDateTime);
begin
  FDate := Value;
  UpdateCalendar;
  Change;
...
function TSuperCalendar.StoreCalendarDate: Boolean;
begin
  Result := not FUseCurrentDate;
end;

function TSuperCalendar.GetDateElement(Index: Integer): Integer;
var
...
procedure TSuperCalendar.SetDateElement(Index: Integer; Value: Integer);
var
  AYear, AMonth, ADay: Word;
begin
  if Value > 0 then
...
procedure TSuperCalendar.SetStartOfWeek(Value: TDayOfWeek);
begin
...
procedure TSuperCalendar.SetUseCurrentDate(Value: Boolean);
begin
...
procedure TSuperCalendar.ChangeMonth(Delta: Integer);
var
  AYear, AMonth, ADay: Word;
  NewDate: TDateTime;
  CurDay: Integer;
...
procedure TSuperCalendar.PrevMonth;
begin
  ChangeMonth(-1);
end;

procedure TSuperCalendar.NextMonth;
begin
  ChangeMonth(1);
end;

procedure TSuperCalendar.NextYear;
begin
  if IsLeapYear(Year) and (Month = 2) and (Day = 29) then Day := 28;
  Year := Year + 1;
end;

procedure TSuperCalendar.PrevYear;
begin
  if IsLeapYear(Year) and (Month = 2) and (Day = 29) then Day := 28;
  Year := Year - 1;
end;

procedure TSuperCalendar.UpdateCalendar;
var
  AYear, AMonth, ADay: Word;
  FirstDate: TDateTime;
begin
...
procedure TSuperCalendar.WMSize(var Message: TWMSize);
var
  GridLines: Integer;
begin
...
procedure Register;
begin
  RegisterComponents('NewPower', [TSuperCalendar]);
end;
