...
uses
  ...
  StdCtrls, Buttons, EanKod, ExtCtrls, EanSpecs, Printers,
...
procedure TEANSetupFmt.BitBtn1Click(Sender: TObject);
var w, h , margin_left, margin_top, margin_right, margin_bottom, space_x, space_y:Double;
    Count : Integer;
    UseNext  : Boolean;
begin
     ...
     margin_right  := StrToFloat(E_RIGHT.Text);
     margin_bottom := StrToFloat(E_BOTTOM.Text);

     space_x       := StrToFloat(E_SPACEX.Text);
     space_y       := StrToFloat(E_SPACEY.Text);

     Count := StrToIntDef(E_COUNT.Text,1);
     UseNext := E_NEXT.Checked;

     Ean.PrintBarcodes(w,h,
...
procedure TEANSetupFmt.E_WIDTHChange(Sender: TObject);
begin
     UpdateLabel;
end;

procedure TEanSetupFmt.UpdateLabel;
