...
type
  ...
  published
    ...
    property Commands  : TSetCommandType read FCommands write FCommands default
             [btInc, btAlt, btExc, btPos, btFlt, btImp, btExp];
  end;
...
begin
  ...
  FCommands  := [btInc, btAlt, btExc, btPos, btFlt, btImp, btExp];
end;
  ...
  for i := 0 to ControlCount -1 do
   begin
     ...
     if (Controls[i] is TButton) or (Controls[i] is TSpeedButton) then
       begin
         if TComponent(Controls[i]).Tag > 6 then
...
begin
  for i := 0 to ControlCount -1 do
   begin
     // Testa se e TButton
     if (Controls[i] is TButton) or (Controls[i] is TSpeedButton) then
        if (TComponent(Controls[i]).Tag <= 6) and (TComponent(Controls[i]).Tag >= 0) then
