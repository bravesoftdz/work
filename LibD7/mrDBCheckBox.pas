...
uses
  ...
  Types, Graphics, Messages, cxDBEdit, cxCheckBox;
type
  TmrDBCheckBox = class(TcxDBCheckBox)
  private
...
procedure Register;
begin
  RegisterComponents('MultiTierDataControls', [TmrDBCheckBox]);
end;

{ TcxDBCheckBox }

procedure TmrDBCheckBox.CMBidimodechanged(var Message: TMessage);
begin
  inherited;
...
procedure TmrDBCheckBox.CMEnabledchanged(var Message: TMessage);
begin
  inherited;
...
procedure TmrDBCheckBox.CMVisiblechanged(var Message: TMessage);
begin
  inherited;
...
constructor TmrDBCheckBox.Create(AOwner: TComponent);
begin
...
procedure TmrDBCheckBox.GetFieldConfig;
begin
...
procedure TmrDBCheckBox.Notification(AComponent: TComponent;
...
procedure TmrDBCheckBox.SetBounds(ALeft, ATop, AWidth, AHeight: Integer);
begin
...
procedure TmrDBCheckBox.SetLabelPosition(const Value: TLabelPosition);
var
  P: TPoint;
begin
...
procedure TmrDBCheckBox.SetLabelSpacing(const Value: Integer);
begin
  FLabelSpacing := Value;
  SetLabelPosition(FLabelPosition);
end;

procedure TmrDBCheckBox.SetLocked(const Value: Boolean);
begin
  FLocked := Value;
...
procedure TmrDBCheckBox.SetName(const Value: TComponentName);
begin
...
procedure TmrDBCheckBox.SetParent(AParent: TWinControl);
begin
...
procedure TmrDBCheckBox.SetRequired(const Value: Boolean);
begin
  FRequired := Value;
  if Value then
    FRequiredLabel.Parent := Self.Parent
...
procedure TmrDBCheckBox.SetupInternalLabel;
begin
