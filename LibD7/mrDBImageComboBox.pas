unit mrDBImageComboBox;

interface

uses
  SysUtils, Classes, Controls, StdCtrls, ExtCtrls, DBCtrls, Messages, Types,
  Graphics, mrBoundLabel, cxControls, cxContainer, cxEdit, cxTextEdit,
  cxMaskEdit, cxDropDownEdit, cxImageComboBox, cxDBEdit;

type
  TmrDBImageComboBox = class(TcxDBImageComboBox)
  private
...
procedure Register;
begin
  RegisterComponents('MultiTierDataControls', [TmrDBImageComboBox]);
end;

{ TmrDBImageComboBox }

procedure TmrDBImageComboBox.CMBidimodechanged(var Message: TMessage);
begin
  inherited;
...
procedure TmrDBImageComboBox.CMEnabledchanged(var Message: TMessage);
begin
  inherited;
...
procedure TmrDBImageComboBox.CMVisiblechanged(var Message: TMessage);
begin
  inherited;
...
constructor TmrDBImageComboBox.Create(AOwner: TComponent);
begin
...
destructor TmrDBImageComboBox.Destroy;
begin
  FMyDataLink.Free;
  inherited Destroy;
end;

procedure TmrDBImageComboBox.Notification(AComponent: TComponent;
...
procedure TmrDBImageComboBox.SetBounds(ALeft, ATop, AWidth,
  AHeight: Integer);
begin
...
procedure TmrDBImageComboBox.SetLabelPosition(const Value: TLabelPosition);
var
  P: TPoint;
begin
...
procedure TmrDBImageComboBox.SetLabelSpacing(const Value: Integer);
begin
  FLabelSpacing := Value;
  SetLabelPosition(FLabelPosition);
end;

procedure TmrDBImageComboBox.SetLocked(const Value: Boolean);
begin
  FLocked := Value;
...
procedure TmrDBImageComboBox.SetName(const Value: TComponentName);
begin
...
procedure TmrDBImageComboBox.SetParent(AParent: TWinControl);
begin
...
procedure TmrDBImageComboBox.SetRequired(const Value: Boolean);
begin
  FRequired := Value;
  if Value then
    FRequiredLabel.Parent := Self.Parent
...
procedure TmrDBImageComboBox.SetupInternalLabel;
begin
