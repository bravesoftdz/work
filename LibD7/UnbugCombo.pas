    ...
    property OnAfterAddUpdate : TNotifyEvent read FOnAfterAddUpdate write FOnAfterAddUpdate;
{
  ...
  TDBUnbugCombo = class(TUnbugCombo)
  private
...
begin
  inherited Click;
  TUnbugCombo(Parent).DoEnter;

  if TUnbugCombo(Parent).LookUpForm.Visible then
    begin
     TUnbugCombo(Parent).LookUpForm.Close;
     TUnbugCombo(Parent).SetFocus;
    end
  else
     TUnbugCombo(Parent).OpenGrid;
end;
...
begin
  ...
  begin
    with Message, TDataSet(Datasource.DataSet) do
      case ScrollCode of
        SB_LINEUP    : TUnbugCombo(ObjectSuperCombo).GetPriorValue;
        SB_LINEDOWN  : TUnbugCombo(ObjectSuperCombo).GetNextValue;
        SB_PAGEUP    : TUnbugCombo(ObjectSuperCombo).GetPriorPage;
        SB_PAGEDOWN  : TUnbugCombo(ObjectSuperCombo).GetNextPage;
        SB_THUMBPOSITION:
          begin
            if IsSequenced then
            begin
              ...
              case Pos of
                0: TUnbugCombo(ObjectSuperCombo).GetFirstValue;
                1: TUnbugCombo(ObjectSuperCombo).GetPriorPage;
                2: Exit;
                3: TUnbugCombo(ObjectSuperCombo).GetNextPage;
                4: TUnbugCombo(ObjectSuperCombo).GetLastValue;
              end;
          end;
        SB_BOTTOM : TUnbugCombo(ObjectSuperCombo).GetLastValue;
        SB_TOP    : TUnbugCombo(ObjectSuperCombo).GetFirstValue;
      end;
    TUnbugCombo(ObjectSuperCombo).FillFieldValues(TUnbugCombo(ObjectSuperCombo).FComboFields);
  end;
end;

{---- TSuperCombo----}

constructor TUnbugCombo.Create( AOwner: TComponent );
begin
  inherited Create( AOwner );
...
destructor TUnbugCombo.Destroy;
begin
  SetLength(FComboFields, 0);
  EditCode := nil;
  LookUpForm.Free;
...
procedure TUnbugCombo.SetSpcWhereClause(Value : String);
begin
  FSpcWhereClause := Value;
  if Assigned(LookUpADOQuery) then LookUpADOQuery.Close;
end;

procedure TUnbugCombo.SetFilterFields(Value : TStringList);
begin
  if FFilterFields <> Value then
     FFilterFields.Assign(Value);
end;

procedure TUnbugCombo.SetFilterValues(Value : TStringList);
begin
  if FFilterValues <> Value then
     FFilterValues.Assign(Value);
end;

procedure TUnbugCombo.ClearFilters;
begin
  FFilterFields.Clear;
  FFilterValues.Clear;
end;

procedure TUnbugCombo.AddFilter(aFields, aValues : array of string);
var
   i : integer;
begin
  ClearFilters;
...
procedure TUnbugCombo.SetBounds(ALeft, ATop, AWidth, AHeight: Integer);
var
...
procedure TUnbugCombo.SetLookUpSource(Value: TDataSource);
begin
  FLookUpSource := Value;
  if Value <> nil then
    begin
...
procedure TUnbugCombo.CreateParams( var Params: TCreateParams );
begin
  inherited CreateParams( Params );
  Params.Style := Params.Style or WS_CLIPCHILDREN;
end;

procedure TUnbugCombo.AdjustComboBounds;
begin
  if not (csDesigning in ComponentState) then
    begin
      try
...
procedure TUnbugCombo.WMSize( var Message: TWMSize );
var
  rectDraw: TRect;
begin
  if not (csDesigning in ComponentState) then
...
Procedure TUnbugCombo.DrawDropDownArrow(Canvas: TCanvas; R: TRect;
                                           State: TButtonState;
                                           Enabled: Boolean;
                                           ControlState: TControlState);
var Flags: Integer;
...
procedure TUnbugCombo.SetShowBtnSelecao(Value : Boolean);
begin
...
procedure TUnbugCombo.SetLookUpQuery;
var
...
procedure TUnbugCombo.LookUpFormShow(Sender : TObject);
var
  rectDraw: TRect;
  FormPoint, P : TPoint;
...
procedure TUnbugCombo.WMKillFocus(var Message: TMessage);
begin
  inherited;
  if not (csDesigning in ComponentState) then
     EditCode.Parent  := Parent;
...
procedure TUnbugCombo.OwnerDeactivate(Sender: TObject);
begin
...
procedure TUnbugCombo.CNKeyDown(var Message: TWMKeyDown);
begin
  if not (csDesigning in ComponentState) then
  begin
...
procedure TUnbugCombo.GetFirstValue;
begin
...
procedure TUnbugCombo.GetLastValue;
begin
...
procedure TUnbugCombo.GetNextValue;
begin
...
procedure TUnbugCombo.GetNextPage;
begin
...
procedure TUnbugCombo.GetPriorValue;
begin
...
procedure TUnbugCombo.GetPriorPage;
begin
...
procedure TUnbugCombo.CMCancelMode(var Message: TCMCancelMode);
begin
  inherited;
...
procedure TUnbugCombo.SetVisible(Value: Boolean);
begin
  FVisible := Value;
...
procedure TUnbugCombo.SetEnabled(Value: Boolean);
begin
  FEnab := Value;
...
procedure TUnbugCombo.SetReadOnly(Value: Boolean);
begin
  FReadOnly := Value;
...
procedure TUnbugCombo.KeyDown(var Key: Word; Shift: TShiftState);
begin
...
procedure TUnbugCombo.KeyPress(var Key: Char);
begin
  inherited;

end;

procedure TUnbugCombo.Change;
begin
  inherited Change;
...
procedure TUnbugCombo.OpenGrid;
var
  lExit : Boolean;
...
procedure TUnbugCombo.LookUpGridClick(Sender : TObject);
begin
   LookUpGrid.Options := LookUpGrid.Options + [dgAlwaysShowSelection];
   SetFocus;
...
procedure TUnbugCombo.LookUpGridDblClick(Sender : TObject);
begin
...
procedure TUnbugCombo.SelectActualValue;
begin
   if FLookUpValue <> '' then
     begin
      ...
      if DataAware then
         TDBUnbugCombo(Self).UpdateData(Self);
...
procedure TUnbugCombo.UpdateValues;
var
   Key : Char;
begin
...
procedure TUnbugCombo.CMExit(var Message: TCMExit);
begin
...
procedure TUnbugCombo.SetExit(AShowing: Boolean = false);
var
  OldText : String;
begin
  if Assigned(LookUpADOQuery) then
...
function  TUnbugCombo.GetLookUpFieldValue : Variant;
begin
...
procedure TUnbugCombo.FillFieldValues(Var AComboFields: TComboFields);
var
  I : Integer;
begin
...
function  TUnbugCombo.GetFieldByName(AFieldName: string) : Variant;
var
  I : Integer;
begin
  Result := Null;
...
procedure TUnbugCombo.SetLookUpValue(Value: String);
begin
  // Define datasources do Browse e query
  SetLookUpQuery;
...
procedure TUnbugCombo.AddUpdateClick(Sender : TObject);
begin
...
procedure TUnbugCombo.CallUpdate;
begin
  if FLookUpValue <> '' then
    begin
...
constructor TDbUnbugCombo.Create(AOwner: TComponent);
begin
  inherited Create(AOwner);
...
destructor TDbUnbugCombo.Destroy;
begin
  FDataLink.Free;
  FDataLink := nil;
  FCanvas.Free;
...
procedure TDbUnbugCombo.Loaded;
begin
  inherited Loaded;
  if (csDesigning in ComponentState) then DataChange(Self);
end;

procedure TDbUnbugCombo.Notification(AComponent: TComponent;
...
procedure TDbUnbugCombo.KeyDown(var Key: Word; Shift: TShiftState);
begin
  inherited KeyDown(Key, Shift);
...
procedure TDbUnbugCombo.KeyPress(var Key: Char);
begin
...
function TDbUnbugCombo.EditCanModify: Boolean;
begin
  Result := FDataLink.Edit;
end;

procedure TDbUnbugCombo.Reset;
begin
  FDataLink.Reset;
  SelectAll;
end;

procedure TDbUnbugCombo.SetFocused(Value: Boolean);
begin
...
procedure TDbUnbugCombo.Change;
begin
  inherited Change;
  FDataLink.Modified;
end;

function TDbUnbugCombo.GetDataSource: TDataSource;
begin
  Result := FDataLink.DataSource;
end;

procedure TDbUnbugCombo.SetDataSource(Value: TDataSource);
begin
  FDataLink.DataSource := Value;
  if Value <> nil then Value.FreeNotification(Self);
end;

function TDbUnbugCombo.GetDataField: string;
begin
  Result := FDataLink.FieldName;
end;

procedure TDbUnbugCombo.SetDataField(const Value: string);
begin
  FDataLink.FieldName := Value;
end;

function TDbUnbugCombo.GetReadOnly: Boolean;
begin
  Result := FDataLink.ReadOnly;
end;

procedure TDbUnbugCombo.SetReadOnly(Value: Boolean);
begin
  inherited ReadOnly := Value;
  FDataLink.ReadOnly := Value;
end;

function TDbUnbugCombo.GetField: TField;
begin
  Result := FDataLink.Field;
end;

procedure TDbUnbugCombo.DataChange(Sender: TObject);
begin
...
procedure TDbUnbugCombo.EditingChange(Sender: TObject);
begin
//  inherited ReadOnly := not FDataLink.Editing;
end;

procedure TDbUnbugCombo.UpdateData(Sender: TObject);
begin
...
procedure TDbUnbugCombo.WMPaste(var Message: TMessage);
begin
  FDataLink.Edit;
  inherited;
end;

procedure TDbUnbugCombo.WMCut(var Message: TMessage);
begin
  FDataLink.Edit;
  inherited;
end;

procedure TDbUnbugCombo.CMEnter(var Message: TCMEnter);
begin
  SetFocused(True);
  inherited;
end;

procedure TDbUnbugCombo.CMExit(var Message: TCMExit);
begin
...
procedure TDbUnbugCombo.WMPaint(var Message: TWMPaint);
var
  Left: Integer;
  Margins: TPoint;
  R: TRect;
...
procedure TDbUnbugCombo.CMGetDataLink(var Message: TMessage);
begin
  Message.Result := Integer(FDataLink);
end;

function TDbUnbugCombo.GetTextMargins: TPoint;
var
  DC: HDC;
  SaveFont: HFont;
  I: Integer;
...
procedure Register;
begin
  RegisterComponents('NewPower', [TUnbugCombo]);
  RegisterComponents('NewPower', [TDBUnbugCombo]);
end;
