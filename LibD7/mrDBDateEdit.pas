...
uses
  ...
  Types, Graphics, Messages, cxDBEdit,cxCalendar;

type
  TmrDBDateEdit = class(TcxDBDateEdit)
  private
...
procedure Register;
begin
  RegisterComponents('MultiTierDataControls', [TmrDBDateEdit]);
end;

{ TmrDBEdit }

procedure TmrDBDateEdit.CMBidimodechanged(var Message: TMessage);
begin
  inherited;
...
procedure TmrDBDateEdit.CMEnabledchanged(var Message: TMessage);
begin
  inherited;
...
procedure TmrDBDateEdit.CMVisiblechanged(var Message: TMessage);
begin
  inherited;
...
constructor TmrDBDateEdit.Create(AOwner: TComponent);
begin
...
procedure TmrDBDateEdit.GetFieldConfig;
begin
...
procedure TmrDBDateEdit.Notification(AComponent: TComponent;
...
procedure TmrDBDateEdit.SetBounds(ALeft, ATop, AWidth, AHeight: Integer);
begin
...
procedure TmrDBDateEdit.SetLabelPosition(const Value: TLabelPosition);
var
  P: TPoint;
begin
...
procedure TmrDBDateEdit.SetLabelSpacing(const Value: Integer);
begin
  FLabelSpacing := Value;
  SetLabelPosition(FLabelPosition);
end;

procedure TmrDBDateEdit.SetLocked(const Value: Boolean);
begin
  FLocked := Value;
...
procedure TmrDBDateEdit.SetName(const Value: TComponentName);
begin
...
procedure TmrDBDateEdit.SetParent(AParent: TWinControl);
begin
...
procedure TmrDBDateEdit.SetRequired(const Value: Boolean);
begin
  FRequired := Value;
  if Value then
    FRequiredLabel.Parent := Self.Parent
...
procedure TmrDBDateEdit.SetupInternalLabel;
begin
