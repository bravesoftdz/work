unit mrDBCurrencyEdit;

interface

uses
  SysUtils, Classes, Controls, StdCtrls, ExtCtrls, mrBoundLabel,
  Types, Graphics, Messages, cxCurrencyEdit,cxDBEdit;

type
  TmrDBCurrencyEdit = class(TcxDBCurrencyEdit)
  private
...
procedure Register;
begin
  RegisterComponents('MultiTierDataControls', [TmrDBCurrencyEdit]);
end;

{ TmrDBCurrencyEdit }

procedure TmrDBCurrencyEdit.CMBidimodechanged(var Message: TMessage);
begin
  inherited;
...
procedure TmrDBCurrencyEdit.CMEnabledchanged(var Message: TMessage);
begin
  inherited;
...
procedure TmrDBCurrencyEdit.CMVisiblechanged(var Message: TMessage);
begin
  inherited;
...
constructor TmrDBCurrencyEdit.Create(AOwner: TComponent);
begin
...
procedure TmrDBCurrencyEdit.GetFieldConfig;
begin
...
procedure TmrDBCurrencyEdit.Notification(AComponent: TComponent;
...
procedure TmrDBCurrencyEdit.SetBounds(ALeft, ATop, AWidth, AHeight: Integer);
begin
...
procedure TmrDBCurrencyEdit.SetLabelPosition(const Value: TLabelPosition);
var
  P: TPoint;
begin
...
procedure TmrDBCurrencyEdit.SetLabelSpacing(const Value: Integer);
begin
  FLabelSpacing := Value;
  SetLabelPosition(FLabelPosition);
end;

procedure TmrDBCurrencyEdit.SetLocked(const Value: Boolean);
begin
  FLocked := Value;
...
procedure TmrDBCurrencyEdit.SetName(const Value: TComponentName);
begin
...
procedure TmrDBCurrencyEdit.SetParent(AParent: TWinControl);
begin
...
procedure TmrDBCurrencyEdit.SetRequired(const Value: Boolean);
begin
  FRequired := Value;
  if Value then
    FRequiredLabel.Parent := Self.Parent
...
procedure TmrDBCurrencyEdit.SetupInternalLabel;
begin
