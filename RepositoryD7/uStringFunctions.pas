...
function IncChars(S: string; const Len: Integer; cChar : Char; bRight: Boolean):String;
function IncSpaces(S: string; const Len: Integer):String;
function IncLeftSpaces(S: string; const Len: Integer):String;
function CopyStringFlage(cIni, cEnd:String; Text:String; Del:Boolean):String;
function IncLeftStringFlage(cIni, cEnd:String; Text:String; Len:Integer):String;
function ReturnNumber(text:String):String;
function SizedStr(S: String; ALength: Integer; C: Char;
  StrPos: TSizedStrPos): String;
function WrapDelimitedText(ALinha: String; cDelimitator: Char; SplitString: String = #13#10): String;
function TruncString(Text: Variant; Lim: Integer): Variant;
function ReadStreamStr(Stream : TStream) : string;
function ReadStreamInt(Stream : TStream) : integer;
function ReplaceInvalidChars(AText: String): String;
function isInvalidChar(text: String): Boolean;
...
begin
  ...
  for i:= 1 to ( length(text) )  do begin
      if ( pos(text[i], invalidChar) > 0 ) then begin
//         text[i] := #0; 
//         showMessage('Invalid character: ' + text[i]);
         result := true;
         break;
      end;
  end;
...
begin
  Result := Null;
  if Text <> Null then
    Result := Copy(VarToStr(Text), 1, Lim);
end;
...
begin
  ...
  if Pos(cIni, Text) = 0 then
     Exit;

  sTemp := Copy(Text, Pos(cIni,Text), (Pos(cEnd,Text)-Pos(cIni,Text)+Length(cEnd)));
  OldText := sTemp;

  sTemp   := CopyStringFlage(cIni,cEnd, sTemp, False);
  sTemp   := IncLeftSpaces(sTemp, Len);

  Result := StringReplace(Text, OldText, sTemp, [rfReplaceAll]);
...
begin
  ...
  if Del then
     Delete(Text, pIni, (pEnd-pIni)+(pLengE))
  else
     begin
     Delete(Text, pEnd, pLengE);
     Delete(Text, pIni, pLengI);
     end;
...
function WrapDelimitedText(ALinha: String; cDelimitator: Char; SplitString: String = #13#10): String;
  function NextIsDelimiter(S: String; C: Char; iPos: Integer): Boolean;
  begin
    Result := ((iPos + 1) > Length(S));
    if not Result then
      Result := (S[iPos + 1] = C);
  end;
...
begin
  ...
  begin
    if (Atual = 1) and not (QuoteChar in QuoteChars) then
      if strTmp[Atual] in QuoteChars then
      begin
        QuoteChar := strTmp[Atual];
        PosQuote1 := Atual;
        Inc(Atual);
      end
      else
    ...
    if PosQuote1 > 0 then
      begin
        if (strTmp[Atual] = QuoteChar) or (Atual = Length(strTmp)) then
          begin
            PosQuote2 := Atual;
            if NextIsDelimiter(strTmp, cDelimitator, PosQuote2) then
            begin
              Result := Result + Copy(strTmp, PosQuote1 + 1, PosQuote2 - 2);
              Delete(strTmp, 1, Atual + 1);
              PosQuote1 := 0;
              if (Length(strTmp) > 0) or (Atual = 1) then
      ...
      begin
        if (strTmp[Atual] = cDelimitator) or (Atual = Length(strTmp)) then
          begin
            if Atual >= 1 then
              if Pos(cDelimitator, strTmp) > 0 then
                Result := Result + Copy(strTmp, 1, Atual - 1)
              else
                Result := Result + Copy(strTmp, 1, Atual);
            Delete(strTmp, 1, Atual);
