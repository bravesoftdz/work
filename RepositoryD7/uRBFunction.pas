...
{ Alex Begin - 2011/07/14 }

{ TSIGRBFileExists class }
  TFileExistsFunction = class(TraSystemFunction)
  public
    class function Category: string; override;
    class function GetSignature: string; override;
    procedure ExecuteFunction(aParams: TraParamList); override;
...
{ TFileExistsFunction }
class function TFileExistsFunction.Category: string;
begin
  Result := 'MR';
end;

procedure TFileExistsFunction.ExecuteFunction(aParams: TraParamList);
var
  sFileName : String;
  lbResult: Boolean;
begin
  GetParamValue(0, sFileName);
  lbResult := FileExists( sFileName );
  SetParamValue(1, lbResult);
end;

class function TFileExistsFunction.GetSignature: string;
begin
  result := 'Function FileExists( psFileName : String ) : Boolean;';
end;
...
initialization
  ...
  raRegisterFunction('FileExists', TFileExistsFunction);
...
finalization
  ...
  raUnRegisterFunction('FileExists');
