...
uses Windows, SysUtils, TlHelp32, Psapi, Classes, shellapi, Registry,
     ActiveX, ShFolder, ComObj,   ShlObj;
...
type
   ...
   function FileExistsNoMatterXbits(arg_fileName: String): Boolean;
...
begin
   ...
   if ( is64BitOs ) then begin // determine if operating system is under x64
      arg_fileName := stringReplace(arg_fileName, 'system32', 'Sysnative', [rfReplaceAll, rfIgnoreCase]);
      result := FileExists(arg_fileName);
   end else begin
           result := FileExists(arg_fileName);
       end;
...
function Is64BitOs: Boolean;
type
  TIsWow64Process = function(handle: THandle; var iswow64: Bool): Bool; stdcall; var hKernel32: Integer;
...
Function ReturnWindowsFolder( piFolderId: Integer = -1 ): String;
Var
