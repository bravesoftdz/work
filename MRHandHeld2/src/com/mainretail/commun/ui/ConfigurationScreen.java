...
public class ConfigurationScreen extends ParentFormOKClose implements KeyboardManager, MouseListener{
	...
	private JLabel lblTransferSale;
	...
	private JTextField edtTransferSale;
	...
	public void initGUI() {
		try {
			...
			}
			{
				...
				getLblIDStore().setText(LanguageResource.getInstance().getText("STORE_NUM"));
			}
			{
			...
			{
				super.getJpnClient().add(getLblTransferSale(), getGridPos(0, 7, true));
				getLblTransferSale().setText(LanguageResource.getInstance().getText("AUTOTRANSFER_SALES"));
				getLblTransferSale().setVisible(false);
			}
			{
				super.getJpnClient().add(getEdtTransferSale(), getGridPos(1, 7, false));
				getEdtTransferSale().setPreferredSize(new java.awt.Dimension(231,20));
				getEdtTransferSale().setVisible(false);
			}
			{
	...
	private void fillValues(){
		...
		getEdtTransferSale().setText(String.valueOf(cfg.getTransfSaleNum()));
	...
	private void save(){
		...
		Config.getInstance().setTransfSaleNum(Integer.parseInt(getEdtTransferSale().getText()));
	...
	public JLabel getLblTransferSale() {
		if (lblTransferSale == null)
			lblTransferSale = new JLabel();
		return lblTransferSale;
	}
	
	public JTextField getEdtTransferSale() {
		if (edtTransferSale == null){
			edtTransferSale = new JTextField();
			edtTransferSale.addMouseListener(this);
		}
		
		return edtTransferSale;
	}
