...
uses uMsgBox, uSystemConst, uParamFunctions, uEncryptFunctions, uSqlFunctions,
     ...
     uFrmImput, uFrmTEFCheque, DateUtils;
...
begin
  ...
  SetLength(sSerial, 20);
  DM.EFiscal1.NumeroSerie(sSerial);
  DM.fModuloFiscal.ECFSerial := Copy(Trim(sSerial),1,20);
...
begin
  ...
  if (dGrandeTotalFile <> 0) and (dGrandeTotalPrinter = dGrandeTotalFile) then
    Result := True;
