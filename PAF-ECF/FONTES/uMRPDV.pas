...
begin
  Result := (True and (PreSale.PreSaleType = tptInvoice)) or HasCupom;
end;
   ...
   case iFunction of
    ...
    begin
      ...
      if not pnlDiscount.Visible then
        if not Password.AquireAccess(41, MSG_INF_NOT_GIVE_DICOUNT, IDUserAuth) then
         Exit;
    ...
    FUNC_TEF:
    begin
      if ((DM.fCashRegister.UseTef <> TEF_NONE)) then
...
begin
   ...
   try
     Result := FrmPDVPayments.Start(MP, ATEFCallInfo);
     if Result then
        begin
        SetMeioPag(MP);
        end;
