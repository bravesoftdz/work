...
begin
  ...
  begin
   if TECFTax(ECFTaxList.Objects[Indice]).ECFTotalizador = '' then
     TECFTax(ECFTaxList.Objects[Indice]).ECFTotalizador := GetECFTotParcial(TECFTax(ECFTaxList.Objects[Indice]).ECFIndex);
   Result := TECFTax(ECFTaxList.Objects[Indice]).ECFTotalizador;
  end;
end;

function TDMPDV.GetECFTotParcial(Aliquota: String): String;
var
  sAliquotas, sTemp : String;
  i : Integer;
begin
  ...
  begin
      ...
      begin
        sTemp := Copy(sAliquotas, 1, 5);
        if (sTemp <> '00000') then
        begin
          if Length(Aliquota) = 4 then
          begin
            if Aliquota = Copy(sTemp, 2, 5) then
            begin
          ...
          begin
            Result := FormatFloat('00',i+1) + 'T' + Copy(sTemp, 2, 5);
            Exit;
          end;
          ...
          Break;
        sAliquotas := Copy(sAliquotas, 6, Length(sAliquotas));
      end;
