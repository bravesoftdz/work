...
const
  ...
  SOFTWARE_MAINRETAIL = 'MainRetail';
  SOFTWARE_STARTUP = 'StartUp';
  SOFTWARE_OFFICEMANAGER = 'Office Manager';


  MR_REPORT_INSTALL = 'MRReportInstall.exe';

  SQL_REG_PATH = 'Software\Microsoft\MSSQLServer\MSSQLServer\CurrentVersion';
  MR_INSTALL_DIR = 'C:\Program Files\MainRetail\';

  BpsArray: array [0..2] of Integer = (14400, 1024000, 5120000);
...
begin
  ...
  SoftwareInfo := TSoftwareInfo.Create;
  SoftwareInfo.Software     := SOFTWARE_MAINRETAIL;
  SoftwareInfo.FileDownload := 'MainRetail.zip';
  SoftwareInfo.ReadOnly     := True;
  SoftwareInfo.WWW          := HTTP_DOWNLOAD;
  SoftwareList.AddObject(SoftwareInfo.Software, SoftwareInfo);

  SoftwareInfo := TSoftwareInfo.Create;
  SoftwareInfo.Software     := SOFTWARE_STARTUP;
  SoftwareInfo.FileDownload := 'MRStartUp.zip';
  SoftwareInfo.ReadOnly     := True;
  SoftwareInfo.WWW          := HTTP_DOWNLOAD;
  SoftwareList.AddObject(SoftwareInfo.Software, SoftwareInfo);

  SoftwareInfo := TSoftwareInfo.Create;
  SoftwareInfo.Software     := SOFTWARE_OFFICEMANAGER;
  SoftwareInfo.FileDownload := 'OfficeManager.zip';
  SoftwareInfo.ReadOnly     := False;
  SoftwareInfo.WWW          := HTTP_DOWNLOAD;
  SoftwareList.AddObject(SoftwareInfo.Software, SoftwareInfo);
...
begin
   Result := True;
   FileName := MR_INSTALL_DIR + '\MRStartUp.exe';
   if FileExists(FileName) then
...
begin
  ...
  1 : begin
      ...
      sPetWizard     := 'Wizard';     
      sStartUp       := 'Start-up installation';
      sMainRetail    := 'Point of Sale software to retail store';
      sOfficeManager := 'Account software';
      end;
  ...
  2 : begin
      ...
      sStartUp       := 'Instalador do sistema';
      sMainRetail    := 'Sistema para genrenciamento de lojas';
      sOfficeManager := 'Sistema financeiro';
      end;
  ...
  3 : begin
      ...
      sPetWizard     := 'Assistente';
      sStartUp       := 'Herramienta de la instalación';
      sMainRetail    := 'Software del punto de venta a la tienda al por menor';
      sOfficeManager := 'Software de la cuenta';
      end;
  end;
  ...
  begin
    ...
    else if TSoftwareInfo(SoftwareList.Objects[i]).Software = SOFTWARE_MAINRETAIL then
      TSoftwareInfo(SoftwareList.Objects[i]).Description := sMainRetail
    else if TSoftwareInfo(SoftwareList.Objects[i]).Software = SOFTWARE_OFFICEMANAGER then
      TSoftwareInfo(SoftwareList.Objects[i]).Description := sOfficeManager
    else if TSoftwareInfo(SoftwareList.Objects[i]).Software = SOFTWARE_STARTUP then
      TSoftwareInfo(SoftwareList.Objects[i]).Description := sStartUp;
  end;
      ...
      if bResult then
        begin
          ...
          begin
            ...
            else if TSoftwareInfo(SoftwareList.Objects[i]).FileDownload = 'MRStartUp.zip' then
              TSoftwareInfo(SoftwareList.Objects[i]).SizeMB := FFilesSize.ReadFloat('Software', 'MRStartUp.zip', 0)
            else if TSoftwareInfo(SoftwareList.Objects[i]).FileDownload = 'MainRetail.zip' then
              TSoftwareInfo(SoftwareList.Objects[i]).SizeMB := FFilesSize.ReadFloat('Software', 'MainRetail.zip', 0)
            else if TSoftwareInfo(SoftwareList.Objects[i]).FileDownload = 'OfficeManager.zip' then
              TSoftwareInfo(SoftwareList.Objects[i]).SizeMB := FFilesSize.ReadFloat('Software', 'OfficeManager.zip', 0);
