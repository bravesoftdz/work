...
BEGIN
	...
	DECLARE Purchase_ItemTax_Cursor CURSOR FOR
		...
		SELECT 
			CT.IDLancamentoTipo,
			IsNull(SUM(ABS(IsNull(PIT.TaxValue,0))),0) as Value
		FROM 
			Pur_PurchaseItemTax PIT
...
BEGIN
	...
	DECLARE Purchase_ItemTax_Cursor CURSOR FOR
		...
		SELECT 
			CT.IDLancamentoTipo,
			IsNull(SUM(ABS(IsNull(PIT.TaxValue,0))),0) * (-1) as Value
		FROM 
			Pur_PurchaseItemTaxRet PIT
			LEFT OUTER JOIN VendorTax VT ON (VT.IDVendorTax = PIT.IDVendorTax)
			LEFT OUTER JOIN TaxCategory CT ON (CT.IDTaxCategory = VT.IDTaxCategory)
...
BEGIN
	...
	SET
		...
		Model.DateLastSellingPrice = CASE IsNull(PUI.NewSalePrice,0) WHEN 0 THEN Model.DateLastSellingPrice ELSE @Date END,
...
BEGIN
	...
	SET
		...
		Model.VendorCost	= IsNull(PUI.NewCostPrice,0),
		Model.DateLastCost	= @Date,
		Model.DateLastSellingPrice = @Date,
...
BEGIN
	...
	BEGIN
		...
		SELECT
			@ModelID,
			@IDPessoa,
			MAX(IsNull(IMV.VendorOrder,0))+1
		FROM
			Inv_ModelVendor IMV
		WHERE
			IMV.IDModel = @ModelID
