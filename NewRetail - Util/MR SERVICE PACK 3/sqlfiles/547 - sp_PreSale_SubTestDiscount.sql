...
BEGIN
	IF (IsNull(@MaxDiscount, 0) = 0) AND (IsNull(@Discount, 0) <= (Round(IsNull(@Value, 0) * IsNull(@FatParcDisc, 0), 2) )) AND (@OP = 'I')
		RETURN 0
	ELSE IF (IsNull(@MaxDiscount, 0) = 0) AND (IsNull(@NewDiscount, 0) <= (IsNull(@Value, 0) * IsNull(@FatParcDisc, 0))) AND (@OP = 'I')
		RETURN 0
