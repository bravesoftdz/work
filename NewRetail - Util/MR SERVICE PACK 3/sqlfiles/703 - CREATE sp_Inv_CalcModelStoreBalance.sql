...
BEGIN
	...
	BEGIN
		...
		BEGIN
			...
			FROM
				Inv_ModelStoreBalance MB
			WHERE
				MB.IDModel = @IDModel
				AND MB.BalanceDate = (	SELECT MAX(MB.BalanceDate)
							FROM Inv_ModelStoreBalance MB
			...
			FROM
				InventoryMov IM
				JOIN InventoryMovType IMT ON (IM.InventMovTypeID = IMT.IDInventMovType)
				LEFT OUTER JOIN (SELECT ModelID, PreInventoryMovID, CONVERT(smalldatetime, CONVERT(varchar, MovDate, 103), 103) MovDate FROM InventoryMov WHERE StoreID = @IDStore AND ModelID = @IDModel) EX ON (IM.ModelID = EX.ModelID AND IM.IDPreInvMovExchange = EX.PreInventoryMovID)
				LEFT OUTER JOIN (SELECT IDModel, BalanceDate, AvgCostOut FROM Inv_ModelStoreBalance WHERE IDStore = @IDStore AND IDModel = @IDModel) EXAVG ON (IM.ModelID = EXAVG.IDModel AND CONVERT(smalldatetime, CONVERT(varchar, EX.MovDate, 103), 103) = EXAVG.BalanceDate)
			WHERE
				IM.StoreID = @IDStore
				AND IM.ModelID = @IDModel
