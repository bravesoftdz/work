...
SELECT
	@BonusMinSaleValue = CAST(PropertyValue as money)
FROM
	Sis_PropertyDomain
WHERE
	Property = 'BonusBuckMinSaleValue'

IF (@SaleValue > @BonusMinSaleValue) AND (@BonusBucks > 0)
BEGIN
	INSERT INTO Sal_RebateDiscount (IDPreSaleCreated, IDPreSaleUsed, DiscountValue)
	VALUES (@PreSaleID, NULL, @BonusBucks)
