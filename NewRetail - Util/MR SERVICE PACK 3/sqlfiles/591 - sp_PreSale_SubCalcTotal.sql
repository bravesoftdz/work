...
SET
	dbo.Invoice.SubTotal =
		CASE
			WHEN LEN(SUBSTRING(cast(@SubTotal as varchar), LEN(cast(cast(@SubTotal as int) as varchar))+2, LEN(cast(@SubTotal as varchar)))) > 2 THEN
				CASE
					WHEN CAST(SUBSTRING(SUBSTRING(cast(@SubTotal as varchar), LEN(cast(cast(@SubTotal as int) as varchar))+2, LEN(cast(@SubTotal as varchar))), 3, 1) as int) > 5 THEN cast(Round(@SubTotal, 2) as money)
					ELSE cast(Round(@SubTotal, 2, 1) as money)
				END
			ELSE cast(Round(@SubTotal, 2) as money)
		END
WHERE
