...
BEGIN
	...
	FROM
		Invoice I
		JOIN Media M ON (I.MediaID = M.IDMedia)
		JOIN vwCommission C ON (C.InventMovTypeID = 1 AND I.IDInvoice = C.DocumentID)
		JOIN Pessoa PSP ON (C.ComissionID = PSP.IDPessoa AND C.IDTipoPessoa = PSP.IDTipoPessoa)
		LEFT OUTER JOIN ComisPaga CP ON (C.IDPreInventoryMov = CP.IDDocumento)
	WHERE
...
BEGIN
	...
	BEGIN
		...
		FROM
			Invoice I
			JOIN Media M ON (I.MediaID = M.IDMedia)
			JOIN vwSaleItem C ON (I.IDPreSale = C.DocumentID AND C.Invoice = 0)
			JOIN Pessoa PSP ON (C.ComissionID = PSP.IDPessoa AND C.IDTipoPessoa = PSP.IDTipoPessoa)
			LEFT OUTER JOIN ComisPaga CP ON (C.IDPreInventoryMov = CP.IDDocumento)
		WHERE
			C.ComissionID = @IDVendedor
