...
SELECT @GeneratePrice = CASE WHEN UPPER(SrvValue) = 'TRUE' THEN 1 ELSE 0 END FROM Param WHERE IDParam = 97
...
@@FETCH_STATUS = 0BEGIN
	...
	FROM 	InventoryMov IM
		...
		JOIN TaxCategory TCP ON (TCP.IDTaxCategory = IsNull(@IDTaxCategory, SG.IDTaxCategory))
	...
	BEGIN
		...
		BEGIN
			-- Pega as constantes
			SELECT @IDLancamentoTipo = CAST(PropertyValue AS INT) FROM Sis_PropertyDomain WHERE Property = 'PriceRecordType'
			SELECT @IDDocumentoTipo = CAST(PropertyValue AS INT) FROM Sis_PropertyDomain WHERE Property = 'PriceDocumentType'
			SELECT @IDPagamentoTipo = CAST(PropertyValue AS INT) FROM Sis_PropertyDomain WHERE Property = 'PricePaymentType'
			SELECT @DiasVencimento = CAST(PropertyValue AS INT) FROM Sis_PropertyDomain WHERE Property = 'PriceDueDateDays'
			SELECT @IDVendorType = IDTipoPessoaRoot FROM Pessoa WHERE IDPessoa = @IDVendorPrice
