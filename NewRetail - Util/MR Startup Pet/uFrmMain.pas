...
begin
    ...
    lbMainRetailInfo.Caption := 'Pet Center will be installed on ';
...
begin
  ...
  if BDExist then
  begin
    SetImgERR(imgCreateDB.Picture.Bitmap, lbDBResume, 'Pet Center already installed on this computer.');
    Result := False;
    Exit;
  end;
  ...
  if bRestore then
     ...
     Try
        ...
        Try
           ...
           if Result then
              sMsg := lbDBResume.Caption + #13#10 + 'Pet Center database successful created.'
        except
           on E: Exception do
              begin
              Result := False;
...
begin
  ...
  Try
     ...
     ExecuteFile(sDefaulEXE, 'PET', '', SW_SHOW);
     Result := True;
